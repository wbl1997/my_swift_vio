...
#ifndef INCLUDE_OKVIS_HYBRIDVIO_HPP_
#define INCLUDE_OKVIS_HYBRIDVIO_HPP_
...
class HybridVio : public VioInterface {
 public:
  ...
  HybridVio(okvis::VioParameters& parameters,
   ...
   * detection part. \param frameIdInSource   0 based frame id in the video or
   * the image folder, if not provided default to -1. \param asKeyframe   Use
   * the new image as keyframe. Not implemented. \warning The frame consumer
   * loop does not support using existing keypoints yet. \warning Already
   * specifying whether this frame should be a keyframe is not implemented yet.
   * \return             Returns true normally. False, if the previous one has
   * not been processed yet.
   */
  virtual bool addImage(const okvis::Time& stamp, size_t cameraIndex,
                        const cv::Mat& image,
                        const std::vector<cv::KeyPoint>* keypoints = 0,
                        int frameIdInSource = -1, bool* asKeyframe = 0);
 ...
 private:
  ...
  struct OptimizationResults {
    ...
    okvis::SpeedAndBiases speedAndBiases;    ///< The speeds and biases.
    Eigen::Matrix<double, 3, 1>
        omega_S;  ///< The rotational speed of the sensor.
    ...
    okvis::MapPointVector
        transferredLandmarks;   ///< Vector of the landmarks that have been
                                ///< marginalized out.
    bool onlyPublishLandmarks;  ///< Boolean to signalise the publisherLoop()
                                ///< that only the landmarks should be published
    int frameIdInSource;
    bool isKeyframe;
    Eigen::Matrix<double, 27, 1>
        vTgTsTa_;  ///< entries of Tg Ts Ta in row major order
    Eigen::Matrix<double, 10, 1>
        vfckptdr_;  ///< camera calibration parameters, fx, fy,cx,cy, k1, k2,
                    ///< p1,p2,td, tr
    const static int stateDim_ = 42 + 3 + 4 + 4 + 2;
    Eigen::Matrix<double, stateDim_, 1>
        vVariance_;  ///< standard deviation of these parameters
  };

  /// @name State variables
  /// @{

  okvis::SpeedAndBiases
      speedAndBiases_propagated_;  ///< The speeds and IMU biases propagated by
                                   ///< the IMU measurements.
  /// \brief The IMU parameters.
  /// \warning Duplicate of parameters_.imu
  okvis::ImuParameters imu_params_;
  okvis::kinematics::Transformation
      T_WS_propagated_;  ///< The pose propagated by the IMU measurements
  ...
  /// \warning Lock lastState_mutex_.
  okvis::SpeedAndBiases lastOptimizedSpeedAndBiases_;
  ...
  std::vector<std::shared_ptr<okvis::threadsafe::ThreadSafeQueue<
      std::shared_ptr<okvis::CameraMeasurement>>>>
      cameraMeasurementsReceived_;
  /// IMU measurement input queue.
  okvis::threadsafe::ThreadSafeQueue<okvis::ImuMeasurement>
      imuMeasurementsReceived_;

  /// Position measurement input queue.
  okvis::threadsafe::ThreadSafeQueue<okvis::PositionMeasurement>
  ...
  okvis::threadsafe::ThreadSafeQueue<std::shared_ptr<okvis::MultiFrame>>
  ...
  okvis::threadsafe::ThreadSafeQueue<VioVisualizer::VisualizationData::Ptr>
      visualizationData_;
  ...
  std::mutex
      positionMeasurements_mutex_;  ///< Lock when accessing imuMeasurements_
  std::mutex frameSynchronizer_mutex_;  ///< Lock when accessing the
                                        ///< frameSynchronizer_.
  ...
  std::mutex lastState_mutex_;  ///< Lock when accessing any of the
                                ///< 'lastOptimized*' variables.
  ...
  std::vector<std::thread>
      keypointConsumerThreads_;  ///< Threads running matchingLoop().
  std::vector<std::thread> matchesConsumerThreads_;  ///< Unused.
  std::thread imuConsumerThread_;  ///< Thread running imuConsumerLoop().
  std::thread
      positionConsumerThread_;     ///< Thread running positionConsumerLoop().
  std::thread gpsConsumerThread_;  ///< Thread running gpsConsumerLoop().
  std::thread magnetometerConsumerThread_;  ///< Thread running
                                            ///< magnetometerConsumerLoop().
  std::thread differentialConsumerThread_;  ///< Thread running
                                            ///< differentialConsumerLoop().
...
#endif /* INCLUDE_OKVIS_HYBRIDVIO_HPP_ */
