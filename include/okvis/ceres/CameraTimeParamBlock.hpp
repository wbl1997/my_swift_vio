...
class CameraTimeParamBlock :
 ...
 public:
  ...
  virtual void plus(const double* x0, const double* Delta_Chi,
    ...
    Eigen::Map<Eigen::Matrix<double, nTimeDim, 1> > x0_plus_Delta_(x0_plus_Delta);
...
//  /// @param[in] x0 Variable.
  ...
  virtual void plusJacobian(const double* /*unused: x*/,
    ...
    Eigen::Map<Eigen::Matrix<double, nTimeMinDim, nTimeMinDim, Eigen::RowMajor> > identity(
        jacobian);
...
//  /// @param[in] x0 Variable.
  ...
  virtual void liftJacobian(const double* /*unused: x*/,
                            double* jacobian) const {
    Eigen::Map<Eigen::Matrix<double, nTimeMinDim, nTimeDim, Eigen::RowMajor> > identity(
        jacobian);
    identity.setIdentity();
  }
