...
/*!
 ...
 S: Sensor (IMU)
 */
class MSCKF2 //: public VioBackendInterface
{
 public:
   ...
   * @return True if successful.
   */
  bool getSpeedAndBias(uint64_t poseId, uint64_t imuIdx, okvis::SpeedAndBiases & speedAndBias) const;
   ...
   * @return True if successful.
   */
  bool setSpeedAndBias(uint64_t poseId, size_t imuIdx, const okvis::SpeedAndBiases & speedAndBias);
...
public://huai
  ...
  struct StateInfo
  {
    ...
    int minimalDim; ///< minimal dimension of this state, which can acctually replace member "exists"
  };
  ...
  {
    T_SCi = 0, ///< Extrinsics as T_SC, in MSCKF, only p_SC is changing, R_SC is contant
    Intrinsic, ///< Intrinsics, for pinhole camera, fx ,fy, cx, cy
    Distortion, ///< Distortion coefficients, for radial tangential distoriton of pinhole cameras, k1, k2, p1, p2, [k3], this ordering is OpenCV style
    TD, ///< time delay of the image timestamp with respect to the IMU timescale, Raw t_Ci + t_d = t_Ci in IMU time,
  ...
  {
    SpeedAndBias = 0, ///< Speed and biases as v in /*S*/W-frame, then b_g and b_a
    TG, ///< T_g, T_s, T_a in row major order as defined in Li icra14 high fedeltiy
    TS,
    TA
  };
  ...
  /// t_{d_j} is the current estimated time offset between the visual and inertial data, after
  /// correcting the initial time offset imageDelay.Therefore, t_{d_j} is set 0 at the beginning
  /// t_j is the timestamp of the state, remains constant after initialization
  /// t_{f_i} = t_j - t_{d_j} + t_d + (v-N/2)t_r/N
  /// here t_d and t_r are the true time offset and image readout time
  /// t_{f_i} is the time feature i is observed
  struct States
  {
    States() : isKeyframe(false), id(0), tdAtCreation(0) {}
    // _timestamp = image timestamp - imageDelay + _tdAtCreation
    States(bool isKeyframe, uint64_t id, okvis::Time _timestamp, okvis::Duration _tdAtCreation= okvis::Duration())
      : isKeyframe(isKeyframe), id(id), timestamp(_timestamp), tdAtCreation(_tdAtCreation) {}
    ...
    Eigen::Matrix<double, 6,1> linearizationPoint; /// first estimate of position r_WB and velocity v_WB
  };
  ...
  okvis::PointMap landmarksMap_; ///< Contains all the current landmarks (synched after optimisation). maps landmarkId(i.e., homogeneous point parameter block id) to MapPoint pointer
...
public:
  ...
  /// print out the most recent state vector and the stds of its elements. This function can be called in
  /// the optimizationLoop, but a better way to save results is use the publisher loop
  ...
  inline void resetInitialPVandStd(const InitialPVandStd & rhs, bool bUseExternalPose = false)
  {
      pvstd_ = rhs;
  ...
  const double imageReadoutTime; // time to read out one image of the rolling shutter camera

  // map from state ID to segments of imu measurements, the imu measurements covers the last state and current state of the id and extends on both sides
  ...
  Eigen::Matrix<double, 4 + cameras::RadialTangentialDistortion::NumDistortionIntrinsics, 1> intrinsicParameters_;
  okvis::cameras::PinholeCamera<okvis::cameras::RadialTangentialDistortion > tempCameraGeometry_;
  ...
  enum ResidualizeCase{NotInState_NotTrackedNow=0, NotToAdd_TrackedNow, ToAdd_TrackedNow, InState_NotTrackedNow,
                       InState_TrackedNow};
  std::vector<std::pair<size_t, ResidualizeCase> > mLandmarkID2Residualize; // each landmark's case of residualize,
  // 0 means a point not in the states is not tracked in current frame,
  // 1 means a point not in states is tracked in current frame but not to be included in states,
  ...
  std::deque< uint64_t > mInCovLmIds; // for each point in the state vector/covariance,
  ...
  bool mbUseExternalInitialPose;
  static const size_t mMaxM = 15; // maximum number of such states, currently set to 15, but can be set dynamically as done in Li, icra14 optimization based ...
...
#include "implementation/msckf2.hpp"
