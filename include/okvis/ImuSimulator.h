 ...
 *@param q02n, nominal trajecotry poses,i.e., control points, q_0^w, q_1^w, ..., q_n^w; N=n+1 poses,
 ...
 *@param samples output each entry: timestamps, acceleration of sensor by combined force in the world frame,
...
void InterpolateIMUData(const std::vector<Sophus::SE3Group<Scalar> > &q02n,const std::vector<Scalar>& times, const Scalar outputFreq,
    ...
    std::vector<Tangent> Omega02np1(lineNum+1); // $\Omega_0, \Omega_1, ..., \Omega_n+1$ where $\Omega_j=log((b_j-1)\b_j)$
...
typedef std::vector<okvis::ImuMeasurement, Eigen::aligned_allocator<okvis::ImuMeasurement> > ImuMeasurementVector;
// implements the horizontal circular and vertical sinusoidal motion of the body frame
// world frame x right, y forward, z up, sit at the circle center
// body frame, at each point on the curve, x outward along the radius, y tangent, z up
// imu frame coincides with body frame
class CircularSinusoidalTrajectory{
protected:
    const double wz; //parameter determining the angular rate of sinusoidal motion in the vertical
...
public:
    ...
    virtual void getTrueInertialMeasurements(const okvis::Time tStart, const okvis::Time tEnd,
                                     okvis::ImuMeasurementDeque& imuMeasurements);

    virtual void getNoisyInertialMeasurements(const okvis::Time tStart, const okvis::Time tEnd,
                                     okvis::ImuMeasurementDeque& imuMeasurements);

    virtual void getTruePoses(const okvis::Time tStart, const okvis::Time tEnd,
                        std::vector<okvis::kinematics::Transformation> & vT_WB);

    virtual void getSampleTimes(const okvis::Time tStart, const okvis::Time tEnd, std::vector<okvis::Time> & vTime);

    // compute angular rate in the global frame
    virtual Eigen::Vector3d computeGlobalAngularRate(const okvis::Time time);

    virtual Eigen::Vector3d computeGlobalLinearAcceleration(const okvis::Time time);

    virtual Eigen::Vector3d computeGlobalLinearVelocity(const okvis::Time time);

    virtual okvis::kinematics::Transformation computeGlobalPose(const okvis::Time time);
};
...
struct TestSetting
{
    ...
    TestSetting(bool _addImuNoise= true, bool _addPriorNoise = true,
        ...
        addImuNoise(_addImuNoise), addPriorNoise(_addPriorNoise),
        addImageNoise(_addImageNoise), useImageObservs(_useImageObservs)
    {}
    std::string print()
    {
        return "addImuNoise " + std::to_string((int)addImuNoise) + " addPriorNoise "+ std::to_string((int)addPriorNoise) +
                " addImageNoise " + std::to_string((int)addImageNoise) + " useImageObservs " + std::to_string((int)useImageObservs);
