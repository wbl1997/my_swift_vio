...
// given a bunch of normalized image measurements of a point, and the camera poses, Pw2ck, estimate the point's position in the world
// implemented with TooN library
/*TooN::Vector<3> Get_X_from_xP_lin(std::vector<TooN::Vector<2> >& vV2Xn,std::vector<TooN::SE3<double>>& vSE3){
...
/**
 * @brief direct linear triangulation method see Hartley and Zisserman 2003, implemented with Eigen and Sophus,
 * however this function does not take care of observations of infinity point, and repetitive static observations of a point
 ...
 * @return
 */
Eigen::Vector4d Get_X_from_xP_lin(std::vector<Eigen::Vector3d >& vV2Xn,std::vector<Sophus::SE3d>& vSE3,
 ...
 * Rank deficiency can occur with identical observations of a point, or pure rotation
 * @param isParallel, checks if the point is at or close to infinity
 * @return
 */
Eigen::Vector4d Get_X_from_xP_lin(std::vector<Eigen::Vector3d >& vV3Xn,std::vector<Sophus::SE3d>& vSE3,
                                  bool& isValid, bool& isParallel);

// two view triangulation using mid-point method as discussed in R Hartley, sec 5.3, "Triangulation",
// COMPUTER VISION AND IMAGE UNDERSTANDING, Vol. 68, No. 2, November, pp. 146–157, 1997
Eigen::Vector3d triangulate2View_midpoint(std::vector<Eigen::Vector2d >& vV2Xn,std::vector<Sophus::SE3d>& vSE3);
// based on alex flint's implementation of triangulation, https://github.com/alexflint/triangulation/blob/master/triangulate.py
// midpoint triangulation of 2 or multiple rays
// it is verified (1)if only two views is used to triangulate a point, triangulate_midpoint and triangulate2view_midpoint gives identical result
// (2) triangulate_midpoint gives identical result as its python counterpart in alex flint's triangulation package
Eigen::Vector3d triangulate_midpoint(std::vector<Eigen::Vector2d >& vV2Xn,std::vector<Sophus::SE3d>& vSE3, int how_many=0);

void triangulate_refine_GN(const std::vector< Eigen::Vector3d> & obs,
                           const std::vector<Sophus::SE3d> & frame_poses,
                           Eigen::Vector3d& old_point, int n_iter);
//autodiff Jacobian
void triangulate_refine(const std::vector< Eigen::Vector3d> & obs,
                         const std::vector<Sophus::SE3d> & frame_poses,
                          Eigen::Vector3d& old_point, int n_iter, int algorithm=0);
//analytic Jacobian
void triangulate_refineJ(const std::vector< Eigen::Vector3d> & obs,
                         const std::vector<Sophus::SE3d> & frame_poses,
                          Eigen::Vector3d& old_point, int n_iter, int algorithm=0);
...
// to refine triangulated results, use LM or Dogleg solver or GN as provided by Ceres solver
// this test verified that (1) the result using autodiff and analytical diff is identical for both LM and Dogleg;
// (2) Gauss-Newton method is very competitive for structure only point optimization compared to LM and Dogleg,
// given good initial estimate such as obtained by two view mid point
void TestGet_X_from_xP_lin();
// this test verifies that CostFunction operator() should only take variables and residuals as arguments,
// excluding fixed parameters and observations
int test_powell();
#endif
