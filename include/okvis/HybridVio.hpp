...
class HybridVio : public VioInterface {
 public:
  ...
  HybridVio(okvis::VioParameters& parameters, okvis::MockVioBackendInterface& estimator,
 ...
 private:
  ...
  struct OptimizationResults {
    ...
    int frameIdInSource;
    bool isKeyframe;
    Eigen::Matrix<double,27,1> vTgTsTa_;  ///<entries of Tg Ts Ta in row major order
    Eigen::Matrix<double, 10,1> vfckptdr_;  ///<camera calibration parameters, fx, fy,cx,cy, k1, k2, p1,p2,td, tr
    const static int stateDim_ = 42 + 3 +4 + 4 +2;
    Eigen::Matrix<double, stateDim_,1> vVariance_; ///<standard deviation of these parameters
  };

  /// @name State variables
  /// @{

  okvis::SpeedAndBiases speedAndBiases_propagated_;     ///< The speeds and IMU biases propagated by the IMU measurements.
  ...
  FullStateCallbackWithAllCalibration fullStateCallbackWithAllCalibration_; ///< Full state and calibration callback function.
