...
public:
 /**
   ...
   * @param[in] linearizationPointAtTStart is the first estimates of position p_WS and velocity v_WS at t_start
   ...
   * the length of integral interval only adversely affect the covariance and jacobian a little.
   */
  static int propagation(const okvis::ImuMeasurementDeque & imuMeasurements,
                         ...
                         Eigen::Matrix<double, ceres::ode::OdoErrorStateDim, ceres::ode::OdoErrorStateDim>* covariance_t = 0,
                         Eigen::Matrix<double, ceres::ode::OdoErrorStateDim, ceres::ode::OdoErrorStateDim>* jacobian = 0,
                         const Eigen::Matrix<double, 6, 1> * linearizationPointAtTStart = 0);
// a copy of the original implementation forreference
  static int propagation_original(const okvis::ImuMeasurementDeque & imuMeasurements,
                         const okvis::ImuParameters & imuParams,
                         okvis::kinematics::Transformation& T_WS,
                         Eigen::Vector3d& v_WS,
                         IMUErrorModel<double> & iem,
  ...
  static int propagationBackward(const okvis::ImuMeasurementDeque & imuMeasurements,
                         const okvis::ImuParameters & imuParams,
                         okvis::kinematics::Transformation& T_WS,
                         Eigen::Vector3d& v_WS,
                         IMUErrorModel<double> & iem,
  ...
  // note this method assumes that the z direction of the world frame is negative gravity direction
  ...
  static int propagation_leutenegger_corrected(const okvis::ImuMeasurementDeque & imuMeasurements,
                            const okvis::ImuParameters & imuParams,
                            okvis::kinematics::Transformation& T_WS,
