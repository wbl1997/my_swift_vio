...
if __name__ == '__main__':
    ...
    for algocode, multipliers in imu_noise_multipliers.items():
        for row in multipliers:
            algoname = '{}_{}_{}'.format(algocode, row[0], row[1])
            options = copy.deepcopy(algo_option_templates[algocode])
            options['sigma_g_c'] *= row[0]
            options['sigma_a_c'] *= row[0]
            options['sigma_gw_c'] *= row[1]
            options['sigma_aw_c'] *= row[1]
            algoname_to_options[algoname] = options
