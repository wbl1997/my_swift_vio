...
opengv::bearingVector_t opengv::relative_pose::HybridFrameRelativeAdapter::getBearingVector1(
    size_t index) const {
  return bearingVectors1_[matches_[index].idxA];
}

// Retrieve the bearing vector of a correspondence in viewpoint 2.
opengv::bearingVector_t opengv::relative_pose::HybridFrameRelativeAdapter::getBearingVector2(
    size_t index) const {
...
opengv::translation_t opengv::relative_pose::HybridFrameRelativeAdapter::getCamOffset1(
    size_t /*index*/) const {
...
opengv::rotation_t opengv::relative_pose::HybridFrameRelativeAdapter::getCamRotation1(
    size_t /*index*/) const {
  //We could also check here for camIndex being 0, because this adapter is made
  //for a single camera only
...
opengv::translation_t opengv::relative_pose::HybridFrameRelativeAdapter::getCamOffset2(
    size_t /*index*/) const {
  //We could also check here for camIndex being 0, because this adapter is made
  //for a single camera only
  return Eigen::Vector3d::Zero();
...
opengv::rotation_t opengv::relative_pose::HybridFrameRelativeAdapter::getCamRotation2(
    size_t /*index*/) const {
  //We could also check here for camIndex being 0, because this adapter is made
  //for a single camera only
  return Eigen::Matrix3d::Identity();
...
size_t opengv::relative_pose::HybridFrameRelativeAdapter::getNumberCorrespondences() const {
