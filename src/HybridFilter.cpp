...
HybridFilter::HybridFilter(
    ...
    : mapPtr_(mapPtr),
      ...
      marginalizationResidualId_(0),imageReadoutTime(readoutTime),
      tempCameraGeometry_(0, 0, 1, 1, 0,0, cameras::RadialTangentialDistortion(0,0,0,0)),
      minValidStateID(0),  
...
void HybridFilter::clearImus(){
    imuParametersVec_.clear();
}

bool HybridFilter::addStates(
        ...
        bool asKeyframe)
{
    ...
    for (size_t i = 0; i < extrinsicsEstimationParametersVec_.size(); ++i) {
        ...
        if(statesMap_.size() > 1){
            ...
            std::shared_ptr<okvis::ceres::CameraTimeParamBlock> trParamBlockPtr(
                        new okvis::ceres::CameraTimeParamBlock(imageReadoutTime, id, correctedStateTime));
    ...
    for (size_t i=0; i<imuParametersVec_.size(); ++i){
        ...
        else
        {
            ...
            const Eigen::Matrix<double,9,1> TS =Eigen::Matrix<double,9,1>::Zero();
            id = IdProvider::instance().newId();
    ...
    // depending on whether or not this is the very beginning, we will construct covariance:
    if (statesMap_.size() == 1) {
        ...
        for (size_t i = 0; i < extrinsicsEstimationParametersVec_.size(); ++i) {
            ...
            covDistortion = Eigen::Matrix<double, ceres::nDistortionDim, ceres::nDistortionDim>::Identity();
    ...
    covarianceAugmented.topLeftCorner(numOldNavImuCamPoseStates, numOldNavImuCamPoseStates) =
            covariance_.topLeftCorner(numOldNavImuCamPoseStates, numOldNavImuCamPoseStates);

    covarianceAugmented.block(0, numOldNavImuCamPoseStates, numOldNavImuCamPoseStates, 9) =
            covariance_.topLeftCorner(numOldNavImuCamPoseStates, 9);

    if(numPointStates > 0){
        covarianceAugmented.topRightCorner(numOldNavImuCamPoseStates, numPointStates) =
                covariance_.topRightCorner(numOldNavImuCamPoseStates, numPointStates);

        covarianceAugmented.bottomLeftCorner(numPointStates, numOldNavImuCamPoseStates) =
                covariance_.bottomLeftCorner(numPointStates, numOldNavImuCamPoseStates);

        covarianceAugmented.block(numOldNavImuCamPoseStates+9, numOldNavImuCamPoseStates, numPointStates, 9) =
...
bool HybridFilter::applyMarginalizationStrategy()
{
    ...
    Eigen::Matrix<double, 3, Eigen::Dynamic> reparamJacobian(3, covDim_); //Jacobians of feature reparameterization due to anchor change
    std::vector<Eigen::Matrix<double, 3, Eigen::Dynamic> > vJacobian; // container of these reparameterizing Jacobians
    std::vector<size_t> vCovPtId; // id in covariance of point features to be reparameterized, 0 based
    std::vector<uint64_t> toRemoveLmIds; // id of landmarks to be removed that are in state
    const size_t numNavImuCamStates = 15 + 27 + 9 + cameras::RadialTangentialDistortion::NumDistortionIntrinsics;
    ...
    for(PointMap::iterator pit = landmarksMap_.begin(); pit != landmarksMap_.end(); ){
        ResidualizeCase residualizeCase = mLandmarkID2Residualize[tempCounter].second;
        if(residualizeCase == NotInState_NotTrackedNow || residualizeCase == InState_NotTrackedNow){
            ...
            {
                OKVIS_ASSERT_TRUE_DBG(Exception, pit->second.anchorStateId>0, "a tracked point in the states not recorded");
            ...
            if(residualizeCase == InState_TrackedNow)
            {
                if(pit->second.anchorStateId < minValidStateID)
                {
                    ...
                    okvis::kinematics::Transformation T_GBa; //transform from the body frame at the anchor frame epoch to the global frame
                    ...
                    okvis::kinematics::Transformation T_GA = T_GBa*T_SC; // anchor camera frame to global frame
                    okvis::kinematics::Transformation T_GC = T_GBc*T_SC;

                    std::shared_ptr<okvis::ceres::HomogeneousPointParameterBlock> hppb =
                            std::static_pointer_cast<okvis::ceres::HomogeneousPointParameterBlock>(mapPtr_->parameterBlockPtr(pit->first));
                    ...
                    size_t startRowA= numNavImuCamStates + 9* mStateID2CovID_[pit->second.anchorStateId];

                    reparamJacobian.block<3,3>(0, startRowA) = jacobian.block<3,3>(0,3);
                    reparamJacobian.block<3,3>(0, startRowC) = jacobian.block<3,3>(0,6);

                    std::deque<uint64_t>::iterator idPos = std::find(mInCovLmIds.begin(), mInCovLmIds.end(), pit->first);
                    OKVIS_ASSERT_TRUE(Exception, idPos != mInCovLmIds.end(), "The tracked landmark is not in mInCovLmIds ");
                    size_t covPtId = idPos - mInCovLmIds.begin();
                    vCovPtId.push_back(covPtId);
                    reparamJacobian.block<3,3>(0, numNavImuCamPoseStates + 3*covPtId) = jacobian.topLeftCorner<3,3>();
    ...
    tempCounter =0;
    Eigen::MatrixXd featureJacobian = Eigen::MatrixXd::Identity(covDim_, covDim_); //Jacobian of all the new states w.r.t the old states
    for(auto it = vJacobian.begin(); it!= vJacobian.end(); ++it, ++tempCounter)
    {
        featureJacobian.block(numNavImuCamPoseStates + vCovPtId[tempCounter]*3, 0, 3, covDim_)= vJacobian[tempCounter];
    }
    if(vJacobian.size())
    {
        covariance_ = (featureJacobian*covariance_).eval()*featureJacobian.transpose();
    }
    ...
    std::vector<size_t> toRemoveIndices; // start indices of removed columns, each interval of size 3
    ...
    size_t startKeptRow =0; // start id(based on the old matrix) of the kept rows
    for(auto it= toRemoveIndices.begin(), itEnd= toRemoveIndices.end(); it!=itEnd; ++it){
        vRowStartInterval.push_back(std::make_pair(startKeptRow, *it - startKeptRow));
        startKeptRow = *it + 3;
    }
    ...
    std::cout <<"min valid state Id "<< minValidStateID << " oldest and latest stateid "<<
                statesMap_.begin()->first<<" "<< statesMap_.rbegin()->first<<  std::endl;
    ...
    for(auto rit = statesMap_.rbegin(); rit != statesMap_.rend();) {
        if (rit->first < minValidStateID){
            ...
            rit->second.global[GlobalStates::T_WS].exists = false; // remember we removed
            rit->second.sensors.at(SensorStates::Imu).at(0).at(
                        ImuSensorStates::SpeedAndBias).exists = false; // remember we removed
            paremeterBlocksToBeMarginalized.push_back(rit->second.global[GlobalStates::T_WS].id);
            paremeterBlocksToBeMarginalized.push_back(rit->second.sensors.at(SensorStates::Imu).at(0).at(
                                                          ImuSensorStates::SpeedAndBias).id);
            mapPtr_->removeParameterBlock(rit->second.global[GlobalStates::T_WS].id);
            mapPtr_->removeParameterBlock(rit->second.sensors.at(SensorStates::Imu).at(0).at(
                                              ImuSensorStates::SpeedAndBias).id);
...
//            std::advance(rit, 1);
//            statesMap_.erase( rit.base() );//unfortunately this deletion does not work for statesMap_
        }
//        else
            ++rit;
    }
...
// Huai: this also can be realized with Quaterniond::FromTwoVectors() c.f. https://github.com/dennisss/mvision
...
//set latest estimates for the assumed constant states which are commonly used in computing Jacobians of all feature observations
void HybridFilter::retrieveEstimatesOfConstants(const cameras::NCameraSystem& oldCameraSystem){

    // X_c and all the augmented states including the last inserted one in which a marginalized point has no observation in it
    //p_B^C, f_x, f_y, c_x, c_y, k_1, k_2, p_1, p_2, [k_3], t_d, t_r, \pi_{B_i}(=[p_{B_i}^G, q_{B_i}^G, v_{B_i}^G]),
    numCamPosePointStates_ = 9 + cameras::RadialTangentialDistortion::NumDistortionIntrinsics + 9* (statesMap_.size()) +
            3*mInCovLmIds.size();
    ...
    getSensorStateEstimateAs<ceres::CameraIntrinsicParamBlock>(
                currFrameId, camIdx, SensorStates::Camera, CameraSensorStates::Intrinsic,intrinsic);
    OKVIS_ASSERT_EQ_DBG(Exception, cameras::RadialTangentialDistortion::NumDistortionIntrinsics,
    ...
    getSensorStateEstimateAs<ceres::CameraDistortionParamBlock>(
                currFrameId, camIdx, SensorStates::Camera, CameraSensorStates::Distortion, distortionCoeffs);
    ...
    getSensorStateEstimateAs<ceres::CameraTimeParamBlock>(
                currFrameId, camIdx, SensorStates::Camera, CameraSensorStates::TD, tdLatestEstimate);
    getSensorStateEstimateAs<ceres::CameraTimeParamBlock>(
                currFrameId, camIdx, SensorStates::Camera, CameraSensorStates::TR, trLatestEstimate);
...
bool HybridFilter::computeHxf(const uint64_t hpbid, const MapPoint & mp,
                              Eigen::Matrix<double, 2, 1> & r_i, Eigen::Matrix<double, 2, Eigen::Dynamic>& H_x,
                              Eigen::Matrix<double, 2, Eigen::Dynamic>& H_f, Eigen::Matrix2d & R_i)
{
    computeHTimer.start();

    Eigen::Vector2d obsInPixel;
    Eigen::Vector4d ab1rho = std::static_pointer_cast<okvis::ceres::HomogeneousPointParameterBlock>(
                mapPtr_->parameterBlockPtr(hpbid))->estimate(); // inverse depth parameterization in the anchor frame, [\alpha= X/Z, \beta= Y/Z, 1, \rho=1/Z]
    const uint64_t currFrameId = currentFrameId();
    const uint64_t anchorId = mp.anchorStateId;
    bool bObservedInCurrentFrame = false;
    for(auto itObs= mp.observations.rbegin(), iteObs= mp.observations.rend(); itObs!=iteObs; ++itObs )
    {
        if(itObs->first.frameId == currFrameId){
            ...
            multiFramePtr->getKeypoint(itObs->first.cameraIndex, itObs->first.keypointIndex, obsInPixel);

            double kpSize = 1.0;
            multiFramePtr->getKeypointSize(itObs->first.cameraIndex, itObs->first.keypointIndex, kpSize);
            R_i(0,0)=(kpSize/8)*(kpSize/8); R_i(1,1)=R_i(0,0); // image pixel noise follows that in addObservation function
            R_i(0,1) =0;  R_i(1,0) =0;
            bObservedInCurrentFrame = true;
            break;
        }
    }
    OKVIS_ASSERT_TRUE(Exception, bObservedInCurrentFrame, "a point in computeHxf should be observed in current frame!");
    ...
    Eigen::Vector2d imagePoint; // projected pixel coordinates of the point in current frame ${z_u, z_v}$ in pixel units
    ...
    Eigen::Matrix<double, 2, 9> J_XBj;  // $\frac{\partial [z_u, z_v]^T}{\partial delta\p_{B_j}^G, \delta\alpha (of q_{B_j}^G), \delta v_{B_j}^G$
    ...
    Eigen::Matrix<double, 2, 3> J_pfi;  // $\frac{\partial [z_u, z_v]^T}{\partial [a, b, \rho]}$
    Eigen::Vector2d J_td;
    Eigen::Vector2d J_tr;
    Eigen::Matrix<double, 2, 9> J_XBa;  // $\frac{\partial [z_u, z_v]^T}{\partial delta\p_{B_a}^G)$
    ImuMeasurement interpolatedInertialData; // inertial data at the feature capture epoch
    ...
    Duration featureTime = Duration(tdLatestEstimate + trLatestEstimate* kpN) - statesMap_.at(currFrameId).tdAtCreation;
    ...
    okvis::kinematics::Transformation T_GA(mp.p_BA_G + T_WBa.r(), mp.q_GA); // anchor frame to global frame
    ...
    lP_T_WB =T_WBj;
    lP_sb = sbj;
    Eigen::Matrix<double, 6,1> posVelFirstEstimate = statesMap_.at(currFrameId).linearizationPoint;
...
#endif
    double rho = ab1rho[3];
    okvis::kinematics::Transformation T_BcA= lP_T_WB.inverse()*T_GA; // anchor frame to the body frame associated with current frame
    ...
    std::deque<uint64_t>::iterator idPos = std::find(mInCovLmIds.begin(), mInCovLmIds.end(), hpbid);
    OKVIS_ASSERT_TRUE(Exception, idPos != mInCovLmIds.end(), "The tracked landmark is not in mInCovLmIds ");
    size_t covPtId = idPos - mInCovLmIds.begin();
                              ...
                              Eigen::Matrix<double, Eigen::Dynamic, 3>* pH_fi)
{
    ...
    //camera states, pose states, excluding feature states, and the velocity dimesnion for the anchor state
    if(pH_fi == NULL){
        ...
        OKVIS_ASSERT_EQ_DBG(Exception, anchorId, (++statesMap_.rbegin())->first,
                            "anchor frame of marginalized point should be the frame preceding current frame");
    }
    else
        OKVIS_ASSERT_EQ_DBG(Exception, anchorId, (statesMap_.rbegin())->first,
    ...
    for(size_t kale =0; kale < numPoses; ++kale)
    {
        ...
        Duration featureTime = Duration(tdLatestEstimate + trLatestEstimate* kpN) - statesMap_.at(poseId).tdAtCreation;
    ...
    OKVIS_ASSERT_EQ_DBG(Exception, numValidObs, frameIds.size(), "Inconsistent number of observations and frameIds");
...
void HybridFilter::updateStates(const Eigen::Matrix<double, Eigen::Dynamic, 1> &deltaX)
{
    ...
    if((deltaX.head<9>() - deltaX.segment<9>(numNavImuCamPoseStates-9)).lpNorm<Eigen::Infinity>() > 1e-8){
        std::cout <<"Warn: Correction to the current states from head and tail "<< deltaX.head<9>().transpose() <<std::endl<<
                    deltaX.segment<9>(numNavImuCamPoseStates-9).transpose()<<std::endl;
    }
//    OKVIS_ASSERT_NEAR_DBG(Exception, (deltaX.head<9>() - deltaX.segment<9>(numNavImuCamPoseStates-9)).lpNorm<Eigen::Infinity>(),
//                          0, 1e-8, "Correction to the current states from head and tail should be identical");
    ...
    //update feature states, correction is  \delta[\alpha, \beta, \rho], stored states are [\alpha, \beta, 1, \rho]
    jack =0;
    size_t lkStart =  51 + nDistortionCoeffDim + 9*statesMap_.size();
    size_t aStart = lkStart - 3; // a dummy initialization
    for(auto iter = mInCovLmIds.begin(), iterEnd= mInCovLmIds.end(); iter!=iterEnd; ++iter, ++jack)
    {
        std::shared_ptr<okvis::ceres::HomogeneousPointParameterBlock> hppb =
                std::static_pointer_cast<okvis::ceres::HomogeneousPointParameterBlock>(mapPtr_->parameterBlockPtr(*iter));
        Eigen::Vector4d ab1rho = hppb->estimate(); // inverse depth parameterization in the anchor frame, [\alpha= X/Z, \beta= Y/Z, 1, \rho=1/Z]
...
// but the current implementation does not account for that.
void HybridFilter::optimize(bool verbose)
{
    ...
    std::vector<Eigen::MatrixXd> vH_o; // each entry (2n-3)x(13+9m), n, number of observations, m, states in the sliding window
    ...
    std::vector<Eigen::Matrix<double, 2, Eigen::Dynamic> > vH_x; // each entry 2x(42+13+ 9m)
    std::vector<Eigen::Matrix<double, 2, Eigen::Dynamic> > vH_f; // each entry 2x(3s_k)
    ...
    OKVIS_ASSERT_EQ_DBG(Exception, covDim_,  51+cameras::RadialTangentialDistortion::NumDistortionIntrinsics+
                        9*statesMap_.size()+3*mInCovLmIds.size(), "Inconsistent covDim and number of states");
    ...
    size_t nMarginalizedFeatures = 0; // features not in state and not tracked in current frame
    size_t nInStateFeatures = 0; // features in state and tracked now
    size_t nToAddFeatures =0; //features tracked long enough and to be included in states
    mLandmarkID2Residualize.clear();

    Eigen::MatrixXd variableCov = covariance_.block(okvis::ceres::ode::OdoErrorStateDim, okvis::ceres::ode::OdoErrorStateDim,
                                                    dimH_o[1], dimH_o[1]); //covariance of camera and pose copy states
    Eigen::MatrixXd variableCov2 = covariance_.block(okvis::ceres::ode::OdoErrorStateDim, okvis::ceres::ode::OdoErrorStateDim,
                                                    dimH_o[1]+9, dimH_o[1]+9); //covariance of camera and pose copy states
    size_t tempCounter = 0;
    ...
    for(auto it = landmarksMap_.begin(); it!=landmarksMap_.end(); ++it, ++tempCounter){
                ...
                if(itObs->first.frameId == currFrameId)
                {
                    if(nNumObs == mMaxM){ // this point is to be included in the states
                        ...
                        OKVIS_ASSERT_LT_DBG(Exception, nNumObs, mMaxM,
                                            "A point not in state should not have consecutive features more than mMaxM.");
        ...
        else
        {
            ...
            for(auto itObs= it->second.observations.rbegin(), iteObs= it->second.observations.rend();
                itObs!=iteObs; ++itObs)
            {
                if(itObs->first.frameId == currFrameId)// point in states are still tracked so far
                {
        ...
        if(toResidualize == NotInState_NotTrackedNow && nNumObs>2) //TODO: is >2 too harsh or lenient?
        {
            ...
            bool isValidJacobian = computeHoi(it->first, it->second, r_oi, H_oi, R_oi, ab1rho);
            if(!isValidJacobian)
                continue;
        ...
        else if(toResidualize == InState_TrackedNow)// compute residual and Jacobian for a observed point which is in the states
        {
            Eigen::Matrix<double, 2, 1> r_i;
            Eigen::Matrix<double, 2, Eigen::Dynamic> H_x;
            Eigen::Matrix<double, 2, Eigen::Dynamic> H_f;
            Eigen::Matrix2d R_i;
            bool isValidJacobian = computeHxf(it->first, it->second, r_i, H_x, H_f, R_i);
            if(!isValidJacobian)
                continue;

            double gamma = r_i.transpose()*(H_x*variableCov2*H_x.transpose() + R_i).inverse()* r_i;
    ...
    if(dimH_o[0] + 2*nInStateFeatures > 0)
    {
        ...
        std::cout << "kalman observation dimH_o and 2*tracked instateFeatures "<< dimH_o[0] <<" "<<2*nInStateFeatures<<std::endl;
        // stack Jacobians and residuals for only marginalized features, prepare for QR decomposition to reduce dimension
        ...
        {
            H_all.block(startRow, 0, 2, numCamPosePointStates_ - numPointStates)= vH_x[jack];
            H_all.block(startRow, numCamPosePointStates_ - numPointStates, 2, numPointStates)= vH_f[jack];
        ...
        Eigen::MatrixXd S = H_all*covariance_.bottomRightCorner(numCamPosePointStates_, numCamPosePointStates_)
                *H_all.transpose() + R_all;

        Eigen::MatrixXd K = (covariance_.bottomRightCorner(covDim_, numCamPosePointStates_)*H_all.transpose())*S.inverse();

        // State correction
        Eigen::Matrix<double, Eigen::Dynamic, 1> deltaX = K * r_all;
        OKVIS_ASSERT_FALSE(Exception, std::isnan(deltaX(0))||std::isnan(deltaX(3)), "nan in kalman filter's correction");
...
#else //Li Mingyang RSS 12 optimization based..., positive semi-definiteness not necessarily maintained
        covariance_ = covariance_ - K * S * K.transpose();
#endif
        if(covariance_.diagonal().minCoeff()<0 )
        {
            std::cout<<"Warn: current diagonal in normal update "<<std::endl<< covariance_.diagonal().transpose() <<std::endl;
        ...
        // another check the copied state should have the same covariance as its source
        const size_t numNavImuCamStates = 15 + 27 + 9 + cameras::RadialTangentialDistortion::NumDistortionIntrinsics;
        const size_t numNavImuCamPoseStates = numNavImuCamStates + 9* statesMap_.size();
        if((covariance_.topLeftCorner(covDim_, 9) - covariance_.block(0, numNavImuCamPoseStates-9, covDim_, 9)).lpNorm<Eigen::Infinity>() > 1e-8){
            std::cout <<"Warn: Covariance of cloned state is not equal to source "<< std::endl;
        }
    ...
    if(nToAddFeatures){
        ///initialize features tracked in all m images of the sliding window, see Li RSS 12 supplement material
        ...
        const size_t numCamPoseStates = numCamPosePointStates_ - 3*mInCovLmIds.size();
        Eigen::MatrixXd variableCov = covariance_.block(okvis::ceres::ode::OdoErrorStateDim, okvis::ceres::ode::OdoErrorStateDim,
                                                        numCamPoseStates, numCamPoseStates); //covariance of camera and pose copy states

        std::vector<uint64_t> toAddLmIds; //id of landmarks to add to the states
        for(PointMap::iterator pit = landmarksMap_.begin(); pit != landmarksMap_.end(); ++pit, ++tempCounter){
            if(mLandmarkID2Residualize[tempCounter].second == ToAdd_TrackedNow ){
                Eigen::Vector4d ab1rho; //[\alpha, \beta, 1, \rho] of the point in the anchor frame, representing either an ordinary point or a ray
                bool isValidJacobian = computeHoi( pit->first, pit->second, r_i, H_i, R_i, ab1rho,&H_fi);
                ...
                std::shared_ptr<okvis::ceres::HomogeneousPointParameterBlock> hppb =
                        std::static_pointer_cast<okvis::ceres::HomogeneousPointParameterBlock>(mapPtr_->parameterBlockPtr(pit->first));
                hppb->setEstimate(ab1rho); //for debugging, we may compare the new and old value of this triangulated point

                okvis::kinematics::Transformation T_GBa; //transform from the body frame at the anchor frame epoch to the world frame
                get_T_WS(currFrameId, T_GBa);
                okvis::kinematics::Transformation T_GA = T_GBa*T_SC0_; // anchor frame to global frame
        ...
        if(nNewFeatures){
            std::cout <<"start initializing features into states "<< nNewFeatures<< std::endl;
            Eigen::MatrixXd H_o(totalObsDim -3*nNewFeatures, numCamPoseStates);
            Eigen::MatrixXd H_1(3*nNewFeatures, numCamPoseStates);
            Eigen::MatrixXd invH_2 = Eigen::MatrixXd::Zero(3*nNewFeatures, 3*nNewFeatures);
            Eigen::MatrixXd R_o = Eigen::MatrixXd::Zero(totalObsDim - 3*nNewFeatures, totalObsDim - 3*nNewFeatures);
            Eigen::MatrixXd R_1 = Eigen::MatrixXd::Zero(3*nNewFeatures, 3*nNewFeatures);
            Eigen::Matrix<double, Eigen::Dynamic, 1> z_1(nNewFeatures*3,1);
            Eigen::Matrix<double, Eigen::Dynamic, 1> z_o(totalObsDim-nNewFeatures*3,1);

            tempCounter =0;
            size_t startRow = 0;
            for(auto it = toAddLmIds.begin(), itEnd= toAddLmIds.end(); it!=itEnd; ++it, ++tempCounter)
            {
                H_o.block(startRow, 0, vH_o[tempCounter].rows(), numCamPoseStates) = vH_o[tempCounter];
                H_1.block(3*tempCounter, 0, 3, numCamPoseStates) = vH_1[tempCounter];
                invH_2.block<3,3>(3*tempCounter, 3*tempCounter) = vH_2[tempCounter].inverse();
                R_o.block(startRow, startRow, vH_o[tempCounter].rows(), vH_o[tempCounter].rows()) = vR_o[tempCounter];
...
//TODO: in case H_o has too many rows, we should update the state and covariance for each feature,
            //or use the qr decomposition approach as in MSCKF2
            //in that case H_o is replaced by T_H, and z_o is replaced by Q_1'*z_o

            Eigen::MatrixXd S = H_o*covariance_.block(okvis::ceres::ode::OdoErrorStateDim, okvis::ceres::ode::OdoErrorStateDim,
                                                      numCamPoseStates, numCamPoseStates)*H_o.transpose() + R_o;

            Eigen::MatrixXd K = (covariance_.block(0, okvis::ceres::ode::OdoErrorStateDim, covDim_, numCamPoseStates)*H_o.transpose())*S.inverse();

            updateCovarianceTimer.start();
            Eigen::MatrixXd Paug(covDim_+ nNewFeatures*3, covDim_ + nNewFeatures*3);
#if 0 //Joseph form
            Eigen::MatrixXd tempMat = Eigen::MatrixXd::Identity(covDim_, covDim_);
            ...
            Paug.topLeftCorner(covDim_, covDim_) = covariance_ - K * S * K.transpose();
#endif
            Eigen::MatrixXd invH2H1= invH_2*H_1;
            Paug.block(covDim_, 0, 3*nNewFeatures, covDim_) = - invH2H1*Paug.block(okvis::ceres::ode::OdoErrorStateDim, 0, numCamPoseStates, covDim_);
            Paug.block(0, covDim_, covDim_, 3*nNewFeatures) = Paug.block(covDim_, 0, 3*nNewFeatures, covDim_).transpose();
            Paug.bottomRightCorner(3*nNewFeatures, 3*nNewFeatures) = - Paug.block(covDim_, okvis::ceres::ode::OdoErrorStateDim,
                    3*nNewFeatures, numCamPoseStates)*invH2H1.transpose() + invH_2*R_1*invH_2.transpose();
            covariance_ = Paug;
            covDim_ = covariance_.rows();
            if(covariance_.diagonal().minCoeff()<0 )
            {
                std::cout<<"Warn: current diagonal in adding points "<<std::endl<< covariance_.diagonal().transpose() <<std::endl;
                covariance_.diagonal() = covariance_.diagonal().cwiseAbs();//TODO: hack is ugly!
                //        OKVIS_ASSERT_GT(Exception, covariance_.diagonal().minCoeff(), 0, "negative covariance diagonal elements");
            }
            // another check the copied state should have the same covariance as its source
            ...
            const size_t numNavImuCamPoseStates = numNavImuCamStates + 9* statesMap_.size();
            if((covariance_.topLeftCorner(covDim_, 9) - covariance_.block(0, numNavImuCamPoseStates-9, covDim_, 9)).lpNorm<Eigen::Infinity>() > 1e-8){
                std::cout <<"Warn: Covariance of cloned state is not equal to source after inserting points "<< std::endl;
            }

            updateCovarianceTimer.stop();
            mInCovLmIds.insert(mInCovLmIds.end(), toAddLmIds.begin(), toAddLmIds.end());
            ...
            deltaX.tail(3*nNewFeatures) = - invH2H1*deltaXo.segment(okvis::ceres::ode::OdoErrorStateDim, numCamPoseStates) + invH_2*z_1;
            OKVIS_ASSERT_FALSE(Exception, std::isnan(deltaX(0))||std::isnan(deltaX(covDim_ - 3*nNewFeatures)), "nan in kalman filter's correction in adding points");
            updateStates(deltaX);
    ...
    /// (1) landmark coordinates are used to predict the points projection in new frames OR
    /// (2) to visualize the points
    tempCounter = 0;
    ...
    for(auto it = landmarksMap_.begin(); it!=landmarksMap_.end();++it, ++tempCounter){
        ...
        if(residualizeCase == NotToAdd_TrackedNow){
            if(it->second.observations.size() < 2) //this happens with a just inserted landmark that has not been triangulated.
                continue;
...
bool HybridFilter::setOptimizationTimeLimit(double timeLimit, int minIterations) {
...
bool HybridFilter::getLandmark(uint64_t landmarkId,
...
bool HybridFilter::setSpeedAndBias(uint64_t poseId, size_t imuIdx, const okvis::SpeedAndBiases & speedAndBias)
{
...
bool HybridFilter::triangulateAMapPoint(const MapPoint & mp,  std::vector<Eigen::Vector2d >& obsInPixel,
                                        ...
                                        const uint64_t& hpbid)
{
...
#if 0
    // point's world position estimated in the feature tracking, for debugging
    ...
    {
        std::cout << obsDirections.size()<< std::endl;
        OKVIS_ASSERT_GT(Exception, obsDirections.size(), 2, "for triangulation obs should be greater than 2");
    }
...
//debugging, compare result of triangulateFast and get_x_from
//    Eigen::Vector4d v4Xhomog2 =Get_X_from_xP_lin(obsDirections, T_CWs, isValid, isParallel);
...
//        if(isParallel)
//        {
//            std::cout <<"a point parallel detect only in Get_X_from_xP_lin "<< v4Xhomog.transpose()<<std::endl;
//            triangulateTimer.stop();
//            return true;
//        }
...
//        std::cout << "v4Xhomog by DLT "<< v4Xhomog2.transpose() <<" by midpoint+GN "<< v4Xhomog.transpose()<< std::endl;
...
void HybridFilter::checkStates(){
    ...
    while(1){
      ...
      if(currentTime.sec > gtCurrState[0] || (currentTime.sec == (uint32_t)gtCurrState[0] &&
    ...
    if(currentTime.sec != currentState[0] || std::fabs(currentTime.nsec - (uint32_t)currentState[1])> 500)
    {
        std::cout <<currentTime.sec <<" "<< currentTime.nsec << " "<<
                     (uint32_t)currentState[0] << " "<< (uint32_t)currentState[1]<<std::endl;

        OKVIS_ASSERT_EQ(Exception, currentTime.sec, (uint32_t)currentState[0], "Not aligned timestamps for ground truth");
        OKVIS_ASSERT_NEAR(Exception, currentTime.nsec, (uint32_t)currentState[1], 500, " Not aligned timestamps for ground truth");
    }
    uint64_t poseId = statesMap_.rbegin()->first;
    kinematics::Transformation T_WS0(currentState.segment<3>(2), Eigen::Quaterniond(currentState[8], currentState[5],currentState[6],currentState[7]));

    // output deviation from ground truth
    std::shared_ptr<ceres::PoseParameterBlock> poseParamBlockPtr =std::static_pointer_cast<ceres::PoseParameterBlock>(
                mapPtr_->parameterBlockPtr(poseId));
    kinematics::Transformation T_WS = poseParamBlockPtr->estimate();
    std::cout<<"time "<< currentTime << " dev in T_WS "<<std::setfill(' ')<< (T_WS.parameters() - T_WS0.parameters()).transpose()<<std::endl;
    // update imu sensor states
    const int imuIdx =0;
    ...
    std::shared_ptr<ceres::SpeedAndBiasParameterBlock> sbParamBlockPtr =std::static_pointer_cast<ceres::SpeedAndBiasParameterBlock>(
                mapPtr_->parameterBlockPtr(SBId));
    SpeedAndBiases sb = sbParamBlockPtr->estimate();
    std::cout<<"sb dev "<< (sb - currentState.segment<9>(9)).transpose()<<std::endl;
    ...
    matT_SC0<<  0.0148655429818, -0.999880929698, 0.00414029679422, -0.0216401454975,
    ...
    distIntrinsic <<-0.28340811217, 0.0739590738929, 0.000193595028569, 1.76187114545e-05;
    ...
    std::shared_ptr<ceres::ShapeMatrixParamBlock> tsParamBlockPtr =std::static_pointer_cast<ceres::ShapeMatrixParamBlock>(
                mapPtr_->parameterBlockPtr(TSId));
    sm = tsParamBlockPtr->estimate();
    std::cout<<"Ts dev "<< (sm - vTgTsTa.segment<9>(9)).transpose()<<std::endl;
    ...
    std::shared_ptr<ceres::PoseParameterBlock> extrinsicParamBlockPtr =std::static_pointer_cast<ceres::PoseParameterBlock>(
                mapPtr_->parameterBlockPtr(extrinsicId));
    kinematics::Transformation T_SC0 = extrinsicParamBlockPtr->estimate();
    std::cout<<"T_SC0 dev "<< (kinematics::Transformation(matT_SC0).parameters() - T_SC0.parameters()).transpose()<<std::endl;

    uint64_t intrinsicId = stateInQuestion.sensors.at(SensorStates::Camera).at(camIdx).at(CameraSensorStates::Intrinsic).id;
    std::shared_ptr<ceres::CameraIntrinsicParamBlock> intrinsicParamBlockPtr =std::static_pointer_cast<ceres::CameraIntrinsicParamBlock>(
                mapPtr_->parameterBlockPtr(intrinsicId));
    Eigen::Matrix<double,4,1> cameraIntrinsics = intrinsicParamBlockPtr->estimate();
    std::cout<<"intrinsic dev "<< (cameraIntrinsics - projIntrinsic).transpose()<<std::endl;
    ...
    std::shared_ptr<ceres::CameraDistortionParamBlock> distortionParamBlockPtr =std::static_pointer_cast<ceres::CameraDistortionParamBlock>(
                mapPtr_->parameterBlockPtr(distortionId));
    Eigen::Matrix<double,nDistortionCoeffDim, 1> cameraDistortion = distortionParamBlockPtr->estimate();
    std::cout<<"distortion dev "<< (cameraDistortion - distIntrinsic).transpose()<<std::endl;
    ...
    setGlobalStateEstimateAs<ceres::PoseParameterBlock>(poseId, GlobalStates::T_WS, T_WS0);
    setSensorStateEstimateAs<ceres::SpeedAndBiasParameterBlock>(poseId, 0, SensorStates::Imu,
                ImuSensorStates::SpeedAndBias, currentState.segment<9>(9));

    setSensorStateEstimateAs<ceres::ShapeMatrixParamBlock>(
                    poseId, 0, SensorStates::Imu, ImuSensorStates::TG,
                vTgTsTa.head<9>());
    ...
    setSensorStateEstimateAs<ceres::ShapeMatrixParamBlock>(
                    poseId, 0, SensorStates::Imu, ImuSensorStates::TA,
                vTgTsTa.tail<9>());
    ...
    setSensorStateEstimateAs<ceres::CameraTimeParamBlock>(
                    poseId, 0, SensorStates::Camera, CameraSensorStates::TD,
                0);
    setSensorStateEstimateAs<ceres::CameraTimeParamBlock>(
                    poseId, 0, SensorStates::Camera, CameraSensorStates::TR,
                0);
    }
}
...
bool HybridFilter::getFrameId(uint64_t poseId, int & frameIdInSource, bool & isKF) const
{
