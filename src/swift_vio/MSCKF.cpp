...
#include <swift_vio/MeasurementJacobianMacros.hpp>
...
bool MSCKF::measurementJacobianAIDPMono(
...
bool MSCKF::measurementJacobianHPPMono(
...
bool MSCKF::featureJacobianGeneric(
  ...
  for (size_t observationIndex = 0; observationIndex < numPoses; ++observationIndex) {
    ...
    swift_vio::MeasurementJacobianStatus status = measurementJacobianGeneric(
        *pointLandmark, tempCameraGeometry, obsInPixel[observationIndex],
        obsCov, observationIndex, pointDataPtr, &J_x, &J_pfi, &J_n, &residual);
    if (status != swift_vio::MeasurementJacobianStatus::Successful) {
      if (status == swift_vio::MeasurementJacobianStatus::
...
swift_vio::MeasurementJacobianStatus MSCKF::measurementJacobianGeneric(
...
int MSCKF::computeStackedJacobianAndResidual(
...
void MSCKF::optimize(size_t /*numIter*/, size_t /*numThreads*/, bool verbose) {
