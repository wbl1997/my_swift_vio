...
{
    if(!stream.is_open())
        std::cerr<<"Cannot open feature track file at "<<file<<"."<<std::endl;
    std::string tempStr;
...
bool TrackResultReader::getNextFrame(const double timeStamp, std::vector<cv::KeyPoint> & keypoints,
                                     ...
                                     const size_t frameId)
{
    ...
    if(timeStamp - currentTime > epsilonTime){
        while(!stream.eof()){
            readKeyPointsFromStream(stream, keypoints_,
                                    mapPointIds_, mapPointPositions_,
                                    currentTime, currentFrameId, trackingStatus, tq_wc);
            ...
            {
                std::cout << "Warn: no entry corresponding to time "<<std::setprecision(12) << timeStamp<<std::endl;
                return false;
            }
        }
        if(stream.eof())
    ...
    else if( std::fabs(timeStamp - currentTime)< epsilonTime)
    {
        ...
        std::cout <<" no valid keypoints for time "<<std::setprecision(12)<< timeStamp <<
                    " which is less than current time for keypoints "<< currentTime<< std::endl;
        return false;
    }
                             ...
                             double & timeStamp, size_t & frameId, int & status, Eigen::Matrix<double,7,1> & tq_wc)
{
    ...
    kp_stream >>timeStamp >> frameId >> status>> kpNum>> tq_wc[0] >>tq_wc[1] >>tq_wc[2]>> tq_wc[3]>>
            tq_wc[4]>>tq_wc[5]>>tq_wc[6];
    ...
    if(status==0){
        for(size_t jack=0; jack< kpNum; ++jack)
        {
            ...
            kp_stream >> kp.pt.x >>kp.pt.y >>kp.size>> kp.angle>> kp.response>>kp.octave>>
                    kp.class_id>> mpId>> dummy3[0]>> dummy3[1] >> dummy3[2];
    ...
    else
    {
        ...
        for(size_t jack=0; jack< kpNum; ++jack)
        {
            ...
            kp_stream >> kp.pt.x >>kp.pt.y >>kp.size>> kp.angle>> kp.response>>kp.octave>>
                    kp.class_id>> mpId>> dummy3[0]>> dummy3[1] >> dummy3[2];
            kp.size=kp.size*8/31; //HACK: to make it compatible with msckf2 optimizer observation noise
