...
// Propagates pose, speeds and biases with given IMU measurements with trapezoid rules
// assume linear change of acceleration and angular rate between epochs
int IMUOdometry::propagation(const okvis::ImuMeasurementDeque & imuMeasurements,
                             const okvis::ImuParameters & imuParams,
                             okvis::kinematics::Transformation& T_WS,
                             Eigen::Vector3d & v_WS,
                             IMUErrorModel<double> & iem,
                             const okvis::Time & t_start, const okvis::Time & t_end,
                             Eigen::Matrix<double, ceres::ode::OdoErrorStateDim, ceres::ode::OdoErrorStateDim>* covariance,
                             Eigen::Matrix<double, ceres::ode::OdoErrorStateDim, ceres::ode::OdoErrorStateDim>* jacobian,
    ...
    {
        std::cout << "front timestamp and start integration time "<< imuMeasurements.front().timeStamp<<" "<<time<<std::endl;
        okvis::Time nowTime = time;
    }
//    OKVIS_ASSERT_LT(Exception, imuMeasurements.front().timeStamp, time, "first entry of imu meas is smaller than start
//    time for integration, examine the data if there are large laps in IMU data and possibly enlarge temporal_imu_data_overlap!");
    ...
    //record the linearization point for position p_WS, and v_WS at two subsequent steps
    ...
    Eigen::Matrix3d C_integral = Eigen::Matrix3d::Zero(); // integrated DCM up to Si expressed in S0 frame
    // $\int_{t_0}^{t_i} \int_{t_0}^{s} R_S^{S_0} dt ds$
    Eigen::Matrix3d C_doubleintegral = Eigen::Matrix3d::Zero(); // double integrated DCM up to Si expressed in S0 frame
    // $\int_{t_0}^{t_i} R_S^{S_0} a^S dt$
    Eigen::Vector3d acc_integral = Eigen::Vector3d::Zero(); // integrated acceleration up to Si expressed in S0 frame
    // $\int_{t_0}^{t_i} \int_{t_0}^{s} R_S^{S_0} a^S dt ds$
    Eigen::Vector3d acc_doubleintegral = Eigen::Vector3d::Zero();// double integrated acceleration up to Si expressed in S0 frame
#define USE_INTEGRAL_JACOBIAN
//empirically, use integrals to compute Jacobians performs much better than use product approach
#ifdef USE_INTEGRAL_JACOBIAN
    // sub-Jacobians
    // $R_{S_0}^W \frac{d^{S_0}\alpha_{l+1}}{d b_g_{l}} = \frac{d^W\alpha_{l+1}}{d b_g_{l}} $
    // for ease of implementation, we compute all the Jacobians with positive increment, thus
    // there can be a sign difference between, for instance, dalpha_db_g and \frac{d^{S_0}\alpha_{l+1}}{d b_g_{(l)}}.
    // This difference is adjusted when putting all Jacobians together
    ...
    Eigen::Matrix<double, ceres::ode::OdoErrorStateDim, ceres::ode::OdoErrorStateDim> P_delta;
    Eigen::Matrix<double, ceres::ode::OdoErrorStateDim, ceres::ode::OdoErrorStateDim> T =
            Eigen::Matrix<double, ceres::ode::OdoErrorStateDim, ceres::ode::OdoErrorStateDim>::Identity();
    ...
    for (okvis::ImuMeasurementDeque::const_iterator it = imuMeasurements.begin();
        ...
        // access out of range element won't happen becsuse the loop breaks once nexttime==t_end
        ...
        const Eigen::Vector3d acc_integral_1 = acc_integral + 0.5*(C*acc_est + C_1*acc_est_1)*dt;
        // rotation matrix double integral:
        C_doubleintegral += 0.5* (C_integral + C_integral_1)*dt; // == C_integral*dt + 0.25*(C + C_1)*dt*dt;
        acc_doubleintegral += 0.5*(acc_integral + acc_integral_1)*dt; //==acc_integral*dt + 0.25*(C + C_1)*acc_S_true*dt*dt;

#ifdef USE_INTEGRAL_JACOBIAN
        // Jacobian parts
        if (jacobian) {
            dalpha_dT_g_1 = dalpha_dT_g + 0.5*dt*(C*iem.invT_g* iem.dmatrix3_dvector9_multiply(omega_est) +
                                                  C_1*iem.invT_g* iem.dmatrix3_dvector9_multiply(omega_est_1));
            dalpha_dT_s_1 = dalpha_dT_s + 0.5*dt*(C*iem.invT_g* iem.dmatrix3_dvector9_multiply(acc_est) +
                                                  C_1*iem.invT_g* iem.dmatrix3_dvector9_multiply(acc_est_1));
            dalpha_dT_a_1 = dalpha_dT_a + 0.5*dt*(C*invTgsa* iem.dmatrix3_dvector9_multiply(acc_est) +
            ...
            dv_db_g_1 = dv_db_g + 0.5*dt*(okvis::kinematics::crossMx(C*acc_est)*C_integral +
                                          okvis::kinematics::crossMx(C_1*acc_est_1)*C_integral_1)* iem.invT_g;

            dv_dT_g_1 = dv_dT_g + 0.5*dt*(okvis::kinematics::crossMx(C*acc_est)* dalpha_dT_g +
                                          okvis::kinematics::crossMx(C_1*acc_est_1)* dalpha_dT_g_1);

            dv_dT_s_1 = dv_dT_s + 0.5*dt*(okvis::kinematics::crossMx(C*acc_est)* dalpha_dT_s +
                                          okvis::kinematics::crossMx(C_1*acc_est_1)* dalpha_dT_s_1);

            dv_dT_a_1 = dv_dT_a + 0.5*dt*(C*iem.invT_a* iem.dmatrix3_dvector9_multiply(acc_est) +
                                          C_1*iem.invT_a*iem.dmatrix3_dvector9_multiply(acc_est_1)) +
                    0.5*dt*(okvis::kinematics::crossMx(C*acc_est)* dalpha_dT_a +
        ...
        // covariance propagation of \delta[p^W, \alpha, v^W, b_g, b_a, \vec[S_g, T_s, S_a]
        if (covariance) {

            lP_1.head<3>() = r_0 + v_WS*Delta_t
                    + C_WS_0*(acc_doubleintegral)
                    - 0.5*g_W*Delta_t*Delta_t;
            lP_1.tail<3>() = v_WS + C_WS_0*(acc_integral_1)-g_W*Delta_t;

            Eigen::Matrix<double,ceres::ode::OdoErrorStateDim, ceres::ode::OdoErrorStateDim> F_delta =
                    Eigen::Matrix<double,ceres::ode::OdoErrorStateDim, ceres::ode::OdoErrorStateDim>::Identity();

            F_delta.block<3,3>(3,9) = - 0.5*dt*(C_1 + C)*iem.invT_g;
            F_delta.block<3,3>(3,12) = 0.5*dt*(C_1 + C)*invTgsa;
            F_delta.block<3,9>(3,15) = - 0.5*dt*(C*iem.invT_g *iem.dmatrix3_dvector9_multiply(omega_est) +
                                                 C_1*iem.invT_g *iem.dmatrix3_dvector9_multiply(omega_est_1));
            F_delta.block<3,9>(3,24) = - 0.5*dt*(C*iem.invT_g *iem.dmatrix3_dvector9_multiply(acc_est)
                                                + C_1*iem.invT_g *iem.dmatrix3_dvector9_multiply(acc_est_1));
            F_delta.block<3,9>(3,33) = 0.5*dt*(C*invTgsa* iem.dmatrix3_dvector9_multiply(acc_est)+
                                               C_1*invTgsa* iem.dmatrix3_dvector9_multiply(acc_est_1));
#ifdef USE_FIRST_ESTIMATE
            F_delta.block<3,3>(6,3) = okvis::kinematics::crossMx( - C_WS_0.transpose()*(lP_1.tail<3>()- lP.tail<3>() +
                                                                                        g_W*dt));//vq
#else
            F_delta.block<3,3>(6,3) = - okvis::kinematics::crossMx(0.5*(C*acc_est + C_1*acc_est_1)*dt);//vq
#endif
            F_delta.block<3,3>(6,9) = 0.25*dt*dt*okvis::kinematics::crossMx(C_1*acc_est_1)*(C + C_1)*iem.invT_g;
            //0.5*dt*(C*acc_S_x*cross + C_1*acc_S_x*cross_1); // S Leuntenegger
            F_delta.block<3,3>(6,12) = -0.5*dt*(C + C_1)*iem.invT_a -
                    0.25*pow(dt,2)* okvis::kinematics::crossMx(C_1*acc_est_1)*(C + C_1)*invTgsa;

            F_delta.block<3,9>(6,15) = - 0.5*dt*okvis::kinematics::crossMx(C_1*acc_est_1)*F_delta.block<3,9>(3,15);
            F_delta.block<3,9>(6,24) = - 0.5*dt*okvis::kinematics::crossMx(C_1*acc_est_1)*F_delta.block<3,9>(3,24);
            F_delta.block<3,9>(6,33) = - 0.5*dt*okvis::kinematics::crossMx(C_1*acc_est_1)*F_delta.block<3,9>(3,33)
                    - 0.5*dt*(C*iem.invT_a *iem.dmatrix3_dvector9_multiply(acc_est) +
                             C_1*iem.invT_a *iem.dmatrix3_dvector9_multiply(acc_est_1));

#ifdef USE_FIRST_ESTIMATE
            F_delta.block<3,3>(0,3) = okvis::kinematics::crossMx( - C_WS_0.transpose()*(lP_1.head<3>()- lP.head<3>() -
                                                                                        lP.tail<3>()*dt + 0.5*g_W*dt*dt));//pq
#else
            ...
            GQG.block<3,3>(3,3) = CinvTg*imuParams.sigma_g_c*imuParams.sigma_g_c*CinvTg.transpose()+
                    CinvTgsa*imuParams.sigma_a_c *imuParams.sigma_a_c*CinvTgsa.transpose();
            GQG.block<3,3>(3,6) = CinvTgsa*imuParams.sigma_a_c*imuParams.sigma_a_c*CinvTa.transpose();
            GQG.block<3,3>(6,3) = GQG.block<3,3>(3,6).transpose();
            GQG.block<3,3>(6,6) = CinvTa*imuParams.sigma_a_c*imuParams.sigma_a_c*CinvTa.transpose();
            GQG.block<3,3>(9,9) = Eigen::Matrix3d::Identity()*imuParams.sigma_gw_c * imuParams.sigma_gw_c;
            GQG.block<3,3>(12,12) = Eigen::Matrix3d::Identity()*imuParams.sigma_aw_c * imuParams.sigma_aw_c;

            GQG_1.block<3,3>(3,3) = CinvTg_1*imuParams.sigma_g_c *imuParams.sigma_g_c*CinvTg_1.transpose()+
                    CinvTgsa_1*imuParams.sigma_a_c * imuParams.sigma_a_c*CinvTgsa_1.transpose();
            GQG_1.block<3,3>(3,6) = CinvTgsa_1*imuParams.sigma_a_c* imuParams.sigma_a_c*CinvTa_1.transpose();
            GQG_1.block<3,3>(6,3) = GQG_1.block<3,3>(3,6).transpose();
            GQG_1.block<3,3>(6,6) = CinvTa_1*imuParams.sigma_a_c * imuParams.sigma_a_c*CinvTa_1.transpose();
            GQG_1.block<3,3>(9,9) = Eigen::Matrix3d::Identity()*imuParams.sigma_gw_c * imuParams.sigma_gw_c;
            GQG_1.block<3,3>(12,12) = Eigen::Matrix3d::Identity()*imuParams.sigma_aw_c * imuParams.sigma_aw_c;

            P_delta.topLeftCorner<15,15>() += 0.5*dt*(F_delta.topLeftCorner<15,15>()*GQG*
                                                      F_delta.topLeftCorner<15,15>().transpose()+GQG_1);
#ifndef USE_INTEGRAL_JACOBIAN
            jacobian->topLeftCorner<9,9>()= F_delta.topLeftCorner<9,9>()*jacobian->topLeftCorner<9,9>();
            jacobian->topRightCorner<9,33>()= F_delta.topLeftCorner<9,9>()*jacobian->topRightCorner<9,33>() +
    ...
    if (jacobian) {
        ...
        Eigen::Matrix<double, ceres::ode::OdoErrorStateDim, ceres::ode::OdoErrorStateDim> & F = *jacobian;
        F.setIdentity(); // holds for all states, including d/dalpha, d/db_g, d/db_a
#ifdef USE_FIRST_ESTIMATE
        lP= *linearizationPointAtTStart;
        F.block<3,3>(0,3) = okvis::kinematics::crossMx( - (lP_1.head<3>()- lP.head<3>() - lP.tail<3>()*Delta_t + 0.5*g_W*Delta_t*Delta_t));//pq
#else
...
#ifdef USE_FIRST_ESTIMATE
        F.block<3,3>(6,3) = okvis::kinematics::crossMx( - (lP_1.tail<3>()- lP.tail<3>() + g_W*Delta_t));//vq
#else
        ...
        F.topLeftCorner<3,ceres::ode::OdoErrorStateDim>() = C_WS_0*jacobian->topLeftCorner<3,ceres::ode::OdoErrorStateDim>();
        F.block<3,ceres::ode::OdoErrorStateDim>(3,0) = C_WS_0*jacobian->block<3,ceres::ode::OdoErrorStateDim>(3,0);
        F.block<3,ceres::ode::OdoErrorStateDim>(6,0) = C_WS_0*jacobian->block<3,ceres::ode::OdoErrorStateDim>(6,0);
        jacobian->topLeftCorner<9, ceres::ode::OdoErrorStateDim>()=F;
#endif
    }
    // overall covariance, if requested
    if (covariance) {
        Eigen::Matrix<double, ceres::ode::OdoErrorStateDim, ceres::ode::OdoErrorStateDim> & P = *covariance;
...
// this is the my original implementation for propagation with Tg Ts Ta, the latest implemenation
// for this purpose derives from this with a little change in trapezoid rules rather than rectangular rule.
//
int IMUOdometry::propagation_original(const okvis::ImuMeasurementDeque & imuMeasurements,
                             const okvis::ImuParameters & imuParams,
                             okvis::kinematics::Transformation& T_WS,
                             Eigen::Vector3d & v_WS,
                             IMUErrorModel<double> & iem,
    ...
    // sub-Jacobians
    // $R_{S_0}^W \frac{d^{S_0}\alpha_{l+1}}{d b_g_{(l)}} = \frac{d^W\alpha_{l+1}}{d b_g_{(l)}} $
    // for ease of implementation, we compute all the Jacobians with incremental plus, thus
    // there can be a sign difference between, for instance
    // dalpha_db_g and \frac{d^{S_0}\alpha_{l+1}}{d b_g_{(l)}}. This difference is adjusted in putting all Jacobians together
    Eigen::Matrix3d dalpha_db_g = Eigen::Matrix3d::Zero();
    Eigen::Matrix3d dalpha_db_a = Eigen::Matrix3d::Zero();\
    Eigen::Matrix<double,3,9> dalpha_dT_g = Eigen::Matrix<double,3,9>::Zero();
    Eigen::Matrix<double,3,9> dalpha_dT_s = Eigen::Matrix<double,3,9>::Zero();
    ...
    for (okvis::ImuMeasurementDeque::const_iterator it = imuMeasurements.begin();
        ...
        if (jacobian) {
            ...
            dalpha_dT_s_1 = dalpha_dT_s + 0.5*dt*(C*iem.invT_g* iem.dmatrix3_dvector9_multiply(acc_est) +
                                                                            C_1*iem.invT_g* iem.dmatrix3_dvector9_multiply(acc_est_1));
            dalpha_dT_a_1 = dalpha_dT_a + 0.5*dt*(C*iem.invT_g* iem.T_s *iem.invT_a* iem.dmatrix3_dvector9_multiply(acc_est) +
                                                                            C_1*iem.invT_g* iem.T_s *iem.invT_a*iem.dmatrix3_dvector9_multiply(acc_est_1));
        ...
        if (covariance) {
            Eigen::Matrix<double,ceres::ode::OdoErrorStateDim, ceres::ode::OdoErrorStateDim> F_delta =
                    Eigen::Matrix<double,ceres::ode::OdoErrorStateDim, ceres::ode::OdoErrorStateDim>::Identity();
            F_delta.block<3,3>(0,3) = -okvis::kinematics::crossMx(0.25*(C*acc_est + C_1*acc_est_1)*dt*dt);
            F_delta.block<3,3>(0,6) = Eigen::Matrix3d::Identity()*dt;
            F_delta.block<3,3>(0,9) = 0.125*dt*dt*dt*okvis::kinematics::crossMx(C_1*acc_est_1)*(C + C_1)*iem.invT_g;
            F_delta.block<3,3>(0,12) = -0.25*dt*dt*(C + C_1)*iem.invT_a -
                    0.125*pow(dt,3)* okvis::kinematics::crossMx(C_1*acc_est_1)*(C + C_1)*iem.invT_g*iem.T_s*iem.invT_a;
            //-C_integral*dt + 0.25*(C + C_1)*dt*dt; //S leuntenegger

            F_delta.block<3,3>(3,9) = - 0.5*dt*(C_1 + C)*iem.invT_g;
            F_delta.block<3,3>(3,12) = 0.5*dt*(C_1 + C)*iem.invT_g*iem.T_s*iem.invT_a;
            F_delta.block<3,9>(3,15) = - 0.5*dt*(C+ C_1)*iem.invT_g *iem.dmatrix3_dvector9_multiply(omega_est_1);
            F_delta.block<3,9>(3,24) = - 0.5*dt*(C+ C_1)*iem.invT_g *iem.dmatrix3_dvector9_multiply(acc_est_1);
            F_delta.block<3,9>(3,33) = 0.5*dt*(C+ C_1)*iem.invT_g *iem.T_s *iem.invT_a * iem.dmatrix3_dvector9_multiply(acc_est_1);

            F_delta.block<3,3>(6,3) = - okvis::kinematics::crossMx(0.5*(C*acc_est + C_1*acc_est_1)*dt);
            F_delta.block<3,3>(6,9) = 0.25*dt*dt*okvis::kinematics::crossMx(C_1*acc_est_1)*(C + C_1)*iem.invT_g;
            //0.5*dt*(C*acc_S_x*cross + C_1*acc_S_x*cross_1); // S Leuntenegger
            F_delta.block<3,3>(6,12) = -0.5*dt*(C + C_1)*iem.invT_a -
                    0.25*pow(dt,2)* okvis::kinematics::crossMx(C_1*acc_est_1)*(C + C_1)*iem.invT_g*iem.T_s*iem.invT_a;

            F_delta.block<3,9>(6,15) = - 0.5*dt*okvis::kinematics::crossMx(C_1*acc_est_1)*F_delta.block<3,9>(3,15);
            F_delta.block<3,9>(6,24) = - 0.5*dt*okvis::kinematics::crossMx(C_1*acc_est_1)*F_delta.block<3,9>(3,24);
            F_delta.block<3,9>(6,33) = - 0.5*dt*okvis::kinematics::crossMx(C_1*acc_est_1)*F_delta.block<3,9>(3,33)
                    - 0.5*dt*(C+ C_1)*iem.invT_a *iem.dmatrix3_dvector9_multiply(acc_est_1);
...
// Propagates pose, speeds and biases with given IMU measurements.
int IMUOdometry::propagationBackward(const okvis::ImuMeasurementDeque & imuMeasurements,
                             const okvis::ImuParameters & imuParams,
                             okvis::kinematics::Transformation& T_WS,
                             Eigen::Vector3d & v_WS,
                             IMUErrorModel<double> & iem,
    ...
    for (okvis::ImuMeasurementDeque::const_reverse_iterator it = imuMeasurements.rbegin();
...
// note the RungeKutta method assumes that the z direction of the world frame is negative gravity direction
int IMUOdometry::propagation_RungeKutta(
        ...
        const okvis::Time& startTime, const okvis::Time& finishTime,
        Eigen::Matrix<double, ceres::ode::OdoErrorStateDim, ceres::ode::OdoErrorStateDim>* P_ptr,
        Eigen::Matrix<double, ceres::ode::OdoErrorStateDim, ceres::ode::OdoErrorStateDim>* F_tot_ptr){

    if(imuMeasurements.begin()->timeStamp> startTime){
        std::cout << "imuMeas begin time and startTime "<< imuMeasurements.begin()->timeStamp
                  << " "<< startTime<<std::endl;
        OKVIS_ASSERT_TRUE(Exception, imuMeasurements.begin()->timeStamp<= startTime,
                          "IMU data do not extend to the previous state epoch");
    }
    OKVIS_ASSERT_TRUE(Exception, imuMeasurements.rbegin()->timeStamp>= finishTime,
    ...
    for(auto iter= imuMeasurements.begin(); iter!= imuMeasurements.end(); ++iter){
        ...
        if(bStarted == false){
            bStarted =true;
            if(iter->timeStamp >= finishTime)// in case the interval of start and finish time is very small
            {
                ceres::ode::integrateOneStep_RungeKutta(
                            iterLast->measurement.gyroscopes, iterLast->measurement.accelerometers,
                            iter->measurement.gyroscopes, iter->measurement.accelerometers, imuParams.g,
                            imuParams.sigma_g_c, imuParams.sigma_a_c, imuParams.sigma_gw_c, imuParams.sigma_aw_c,
                            (finishTime - startTime).toSec(), p_WS_W, q_WS, speedAndBias, vTgTsTa, P_ptr, F_tot_ptr);
            ...
            ceres::ode::integrateOneStep_RungeKutta(
                        ...
                        iter->measurement.gyroscopes, iter->measurement.accelerometers, imuParams.g,
                        imuParams.sigma_g_c, imuParams.sigma_a_c, imuParams.sigma_gw_c, imuParams.sigma_aw_c,
                        (iter->timeStamp - startTime).toSec(), p_WS_W, q_WS, speedAndBias, vTgTsTa, P_ptr, F_tot_ptr);
        ...
        else
        {
            if(iter->timeStamp >= finishTime)
            {
                ceres::ode::integrateOneStep_RungeKutta(
                            ...
                            iter->measurement.gyroscopes, iter->measurement.accelerometers, imuParams.g,
                            imuParams.sigma_g_c, imuParams.sigma_a_c, imuParams.sigma_gw_c, imuParams.sigma_aw_c,
                            (finishTime - iterLast->timeStamp).toSec(), p_WS_W, q_WS, speedAndBias, vTgTsTa, P_ptr, F_tot_ptr);
                ++numUsedImuMeasurements;
                break;
            }
            ...
            ceres::ode::integrateOneStep_RungeKutta(
                        ...
                        iter->measurement.gyroscopes, iter->measurement.accelerometers, imuParams.g,
                        imuParams.sigma_g_c, imuParams.sigma_a_c, imuParams.sigma_gw_c, imuParams.sigma_aw_c,
                        (iter->timeStamp - iterLast->timeStamp).toSec(), p_WS_W, q_WS, speedAndBias, vTgTsTa, P_ptr, F_tot_ptr);
        }
        iterLast = iter;
        ++numUsedImuMeasurements;
    }
...
int IMUOdometry::propagationBackward_RungeKutta(
    ...
    OKVIS_ASSERT_TRUE(Exception, imuMeasurements.begin()->timeStamp<= finishTime,
                      "Backward: IMU data do not extend to the current estimated epoch");
    OKVIS_ASSERT_TRUE(Exception, imuMeasurements.rbegin()->timeStamp>= startTime,
    ...
    for(auto iter= imuMeasurements.rbegin(); iter!= imuMeasurements.rend(); ++iter){
        if(iter->timeStamp >= startTime)
        {
            iterLast = iter;
            continue;
        ...
        if(bStarted == false){
            ...
            if(iter->timeStamp <= finishTime)// in case the interval of start and finish time is very small
            {
                ceres::ode::integrateOneStepBackward_RungeKutta(
                            ...
                            iterLast->measurement.gyroscopes, iterLast->measurement.accelerometers, imuParams.g,
                            imuParams.sigma_g_c, imuParams.sigma_a_c, imuParams.sigma_gw_c, imuParams.sigma_aw_c,
                            (startTime - finishTime).toSec(), p_WS_W, q_WS, speedAndBias, vTgTsTa);
                ++numUsedImuMeasurements;
                break;
            }
            ...
            ceres::ode::integrateOneStepBackward_RungeKutta(
                        ...
                        iterLast->measurement.gyroscopes, iterLast->measurement.accelerometers, imuParams.g,
                        imuParams.sigma_g_c, imuParams.sigma_a_c, imuParams.sigma_gw_c, imuParams.sigma_aw_c,
                        (startTime -iter->timeStamp).toSec(), p_WS_W, q_WS, speedAndBias, vTgTsTa);
        ...
        else
        {
            if(iter->timeStamp <= finishTime)
            {
                ceres::ode::integrateOneStepBackward_RungeKutta(
                            ...
                            iterLast->measurement.gyroscopes, iterLast->measurement.accelerometers, imuParams.g,
                            imuParams.sigma_g_c, imuParams.sigma_a_c, imuParams.sigma_gw_c, imuParams.sigma_aw_c,
                            (iterLast->timeStamp -finishTime).toSec(), p_WS_W, q_WS, speedAndBias, vTgTsTa);
                ++numUsedImuMeasurements;
                break;
            }
            ...
            ceres::ode::integrateOneStepBackward_RungeKutta(
                        ...
                        iterLast->measurement.gyroscopes, iterLast->measurement.accelerometers, imuParams.g,
                        imuParams.sigma_g_c, imuParams.sigma_a_c, imuParams.sigma_gw_c, imuParams.sigma_aw_c,
                        (iterLast->timeStamp - iter->timeStamp).toSec(), p_WS_W, q_WS, speedAndBias, vTgTsTa);
        }
        iterLast = iter;
        ++numUsedImuMeasurements;
    }
...
void IMUOdometry::interpolateInertialData(const okvis::ImuMeasurementDeque & imuMeas,
                                          IMUErrorModel<double>& iem,
                                          const okvis::Time & queryTime, okvis::ImuMeasurement& queryValue)
{
    auto iterLeft = imuMeas.begin(), iterRight = imuMeas.end();
    OKVIS_ASSERT_TRUE_DBG(Exception, iterLeft->timeStamp <= queryTime, "Imu measurements has wrong timestamps");
...
// Propagates pose, speeds and biases with given IMU measurements.
int IMUOdometry::propagation_leutenegger_corrected(const okvis::ImuMeasurementDeque & imuMeasurements,
                          const okvis::ImuParameters & imuParams,
                          okvis::kinematics::Transformation& T_WS,
  ...
  Eigen::Matrix<double,15,15> P_delta = /*Eigen::Matrix<double,15,15>::Zero()*/ *covariance;
