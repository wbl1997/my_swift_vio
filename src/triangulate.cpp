...
Eigen::Vector4d Get_X_from_xP_lin(std::vector<Eigen::Vector3d >& vV3Xn,std::vector<Sophus::SE3d>& vSE3,
    ...
    for( int k = 0; k<K; ++k){
        rhs(2*k)=0; rhs(2*k+1)=0;
        A.row(2*k).head<3>()=vV3Xn[k][0]*vSE3[k].rotationMatrix().row(2)-vSE3[k].rotationMatrix().row(0);
        A(2*k,3)=vV3Xn[k][0]*vSE3[k].translation()[2]-vSE3[k].translation()[0];
        A.row(2*k+1).head<3>()=vV3Xn[k][1]*vSE3[k].rotationMatrix().row(2)-vSE3[k].rotationMatrix().row(1);
        A(2*k+1,3)=vV3Xn[k][1]*vSE3[k].translation()[2]-vSE3[k].translation()[1];
    }
    Eigen::JacobiSVD<Eigen::MatrixXd> svdM(A, Eigen::ComputeThinV);
    if(pSingular)
        *pSingular = svdM.singularValues();
...
Eigen::Vector3d triangulate2View_midpoint(std::vector<Eigen::Vector2d >& vV2Xn,std::vector<Sophus::SE3d>& vSE3)
{
    ...
    Matrix3d rel_rot= (vSE3[0].unit_quaternion()*vSE3[1].unit_quaternion().conjugate()).toRotationMatrix();
    ...
    Eigen::Matrix3d rot= vSE3[0].unit_quaternion().conjugate().toRotationMatrix();
...
Eigen::Matrix<double, 2,3> householder(Eigen::Vector3d x){
    ...
    a(minIndex) =1;
    Vector3d u = x.cross(a); u.normalize();
    Vector3d v = x.cross(u); v.normalize();
    Matrix<double, 3,2> res;
    res.col(0)= u;
    res.col(1)= v;
    return res.transpose();
}
Eigen::Vector3d triangulate_midpoint(std::vector<Eigen::Vector2d >& vV2Xn,std::vector<Sophus::SE3d>& vSE3, int num_elements)
{
...
// each observation is in image plane z=1, (\bar{x}, \bar{y}, 1), each of frame_poses is Tw2c(i)
// old_point stores initial position and optimized position
// refine point position with Gauss Newton method \Delta{x}= -(J^TJ)^{-1}J^T (y-f(x)), J=\frac{\partial y-f(x)}{\partial x}
// x_{new}= x+ \Delta{x}
...
#endif
        // stop when converged
        if(norm_max(dp) <= (max_elem+EPS)*EPS)
            break;
    }
    old_point =pos;
#ifdef POINT_OPTIMIZER_DEBUG
...
// parameterized using 7 parameters: 4 for rotation, 3 for translation, qxyzw, txyz
// assume observations are at the z=1 image plane
// Warning: use templated whole class causes type conversion issues in autodiff
struct ReprojectionError {
    ReprojectionError(Sophus::SE3Group<double> _Tw2c,  Eigen::Matrix<double, 2, 1 > _observed)
        : Tw2c(_Tw2c),observed(_observed) {}
    template<typename T>
    bool operator()(const T* const point,
                    T* residuals) const {
        typename Eigen::Matrix<T, 3, 1, Eigen::ColMajor>::ConstMapType v3point(point);
    ...
    static ceres::CostFunction* Create(const Sophus::SE3Group<double> Tw2c,
...
void triangulate_refine(const std::vector< Eigen::Vector3d> & obs,
    ...
    options.trust_region_strategy_type = algorithm==0? ceres::LEVENBERG_MARQUARDT:ceres::DOGLEG;
...
public:
    ReprojectionErrorJ(Sophus::SE3Group<double> _Tw2c,  Eigen::Matrix<double, 2, 1 > _observed)
        : Tw2c(_Tw2c),observed(_observed) {}
    virtual ~ReprojectionErrorJ(){}

    virtual bool Evaluate(double const * const* point,
                    double * residuals,
                          double **jacobians) const {
        typename Eigen::Matrix<double, 3, 1, Eigen::ColMajor>::ConstMapType v3point(point[0]);
...
void triangulate_refineJ(const std::vector< Eigen::Vector3d> & obs,
    ...
    for (size_t i = 0; i < obs.size(); ++i) {
        ...
        // image location and compares the reprojection against the observation.
        Eigen::Vector2d v2InPlane= project2d(obs[i]);
        ceres::CostFunction* cost_function = new ReprojectionErrorJ(frame_poses[i], v2InPlane);
        problem.AddResidualBlock(cost_function,
                                 NULL/* squared loss */,
                                 old_point.data());
    }
