...
okvis::cameras::CameraBase::ProjectionStatus PointLandmarkSimulationRS::projectLandmark(
  ...
  while (numIter < maxIter && std::fabs(dt) > tol) {
      ...
      Eigen::Vector4d point_C = T_SCi.inverse() *
                                T_WBt.inverse() * hpW;
    ...
    for (size_t j = 0; j < homogeneousPoints.size(); ++j) {
      ...
      if (tr > 1e-6) {
        status = projectLandmark(
              simulatedTrajectory, homogeneousPoints[j], *(cameraSystemRef->T_SC(i)),
              cameraSystemRef->cameraGeometry(i), trueCentralRowEpoch, &projection);
      } else {
