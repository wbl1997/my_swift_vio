...
class CameraObservationJacobianTest {
 public:
  ...
  CameraObservationJacobianTest(CameraObservationOptions coo)
      : coo_(coo),
        nextBlockIndex_(1u),
        papLocalParameterization_(new msckf::ParallaxAngleParameterization()) {
  ...
  void addLandmark(std::shared_ptr<msckf::PointLandmark> pl);
 ...
 private:
  ...
  std::vector<std::shared_ptr<msckf::PointLandmark>> visibleLandmarks_;
...
void CameraObservationJacobianTest::addLandmark(
    std::shared_ptr<msckf::PointLandmark> pl) {
  problem_->AddParameterBlock(pl->data(),
                             msckf::ParallaxAngleParameterization::kGlobalDim);
...
public:
 ...
 NumericJacobian(
     ...
     const std::vector<std::shared_ptr<okvis::ceres::PoseParameterBlock>>&
         poseBlocks,
     std::shared_ptr<msckf::PointLandmark> pointLandmark,
     std::shared_ptr<okvis::ceres::PoseParameterBlock> extrinsicBlock,
     ...
     : costFunctionPtr_(costFunctionPtr),
       ...
       parameters_{poseBlocks_[2]->parameters(),
                   ...
                   cameraParameterBlocks_[2]->parameters(),
                   cameraParameterBlocks_[3]->parameters(),
                   speedAndBiasBlocks_[2]->parameters(),
                   speedAndBiasBlocks_[0]->parameters(),
                   speedAndBiasBlocks_[1]->parameters()},
 ...
 std::vector<std::shared_ptr<okvis::ceres::PoseParameterBlock>> poseBlocks_;
 std::shared_ptr<msckf::PointLandmark> pointLandmark_;
...
void CameraObservationJacobianTest::verifyJacobians(
  ...
  for (int j = 0; j < 3; ++j) {
    nj.computeNumericJacobianForPose(j, &de_deltaTWB_numeric[maj_to_jma_index[j]],
                                     &de_deltaTWB_minimal_numeric[maj_to_jma_index[j]]);
    nj.computeNumericJacobianForSpeedAndBias(
        j, &de_dSpeedAndBias_numeric[maj_to_jma_index[j]]);
  }
  ...
  for (int j = 0; j < 3; ++j) {
    ...
    EXPECT_LT((de_dGyroBias_numeric - de_dGyroBias).lpNorm<Eigen::Infinity>(),
              5e-2)
