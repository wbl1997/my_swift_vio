...
// verify that given points' estimated coordinates are indeed unreasonably flipped in triangulateFast
void testTriangulateFast0()
{
    ...
    OKVIS_ASSERT_LT(Exception, (v4Xhomog- precomputed).norm(), 2e-5, "Triangulation result with flip on should be the same ");
    ...
    v4Xhomog = triangulateFastDebug(
                ...
                obsA.normalized(), p_AB,  // center of B in A coordinates
                obsB.normalized(), sigmaR, isValid, isParallel);
    std::cout <<"invalid triangulation with result valid? parallel? "<< isValid <<" "<< isParallel<<" "<< v4Xhomog.transpose() <<std::endl;

}

// see how okvis::triangulation::triangulateFast and Get_X_from_xP_lin deal with A, an ordinary point,
// B, identical observations of an ordinary point, C, an infinity point
void testTriangulateFast(){

    Eigen::Vector3d truePoint( 2.31054434, -1.58786347 , 9.79390227);
    std::vector<Eigen::Vector3d> obsDirections(3);
    ...
    Eigen::Matrix3d rot;
    rot<< 0.99469755, -0.02749299, -0.09910054,
            0.02924625,  0.99943961,  0.0162823,
            0.09859735, -0.01909428,  0.9949442;
    Eigen::Vector3d tcinw(0.37937094, -1.06289834,  1.93156378);
    std::vector<okvis::kinematics::Transformation> T_CWs(3);

    T_CWs[0]= okvis::kinematics::Transformation(-rot*tcinw, Eigen::Quaterniond(rot));

    rot<< 0.99722659, -0.0628095,   0.03992603,
            0.0671776 ,  0.99054536 ,-0.11961209,
            -0.03203577,  0.1219625 ,  0.99201757;
    tcinw<<0.78442247,  0.69195074, -0.10575422;

    T_CWs[1]= okvis::kinematics::Transformation(-rot*tcinw, Eigen::Quaterniond(rot));

    rot<< 0.99958901 , 0.01425856 , 0.02486967,
            -0.01057666,  0.98975966, -0.14235147,
            -0.02664472,  0.14202993 , 0.98950369;
    tcinw<< 0.35451434, -0.09596801,  0.30737151;

    T_CWs[2]= okvis::kinematics::Transformation(-rot*tcinw, Eigen::Quaterniond(rot));
    ...
    Eigen::Vector4d v4Xhomog1 = okvis::triangulation::triangulateFast(
                ...
                (T_AW.C().transpose()*obsDirections.front()).normalized(), T_BW.inverse().r(),  // center of B in W coordinates
                (T_BW.C().transpose()*obsDirections.back()).normalized(), sigmaR, isValid, isParallel);
    v4Xhomog1/= v4Xhomog1[3];
    /// triangulate the point w.r.t the A frame
    Eigen::Vector4d v4Xhomog2 = okvis::triangulation::triangulateFast(
                Eigen::Vector3d(0,0,0),  // center of A in A coordinates
                (obsDirections.front()).normalized(), (T_AW*T_BW.inverse()).r(),  // center of B in A coordinates
                (T_AW.C()*T_BW.C().transpose()*obsDirections.back()).normalized(), sigmaR, isValid, isParallel);
    v4Xhomog2 = T_AW.inverse()* v4Xhomog2;
    v4Xhomog2/= v4Xhomog2[3];

    std::cout <<"True point "<<truePoint.transpose()<<std::endl;
    std::cout <<"2 homog points solutions "<< " "<< v4Xhomog1.transpose() <<
                " "<< v4Xhomog2.transpose()<< std::endl;
    OKVIS_ASSERT_LT(Exception, (v4Xhomog1 - v4Xhomog2).norm(), 1e-7, "results from the two triangulateFast should be the same");

    /// triangulate the point with SVD DLT
    std::vector<Eigen::Vector3d> allObs = obsDirections;
    std::vector<Sophus::SE3d> se3_CWs(3);
    ...
    std::cout <<"eigen svd DLT result of a normal point "<< (v4Xhomog/v4Xhomog[3]).transpose() <<std::endl;
    ...
    std::cout <<"eigen svd DLT result after copying the first observation to third "<<std::endl<<
    ...
    std::cout <<"eigen svd DLT result after copying the first observation to second and third "<<std::endl<<
                (v4Xhomog/v4Xhomog[3]).transpose() <<std::endl;
    std::cout <<"singular values "<<sv.transpose() <<std::endl;
    ...
    v4Xhomog2 = okvis::triangulation::triangulateFast(
                Eigen::Vector3d(0,0,0),  // center of A in A coordinates
                (obsDirections.front()).normalized(), Eigen::Vector3d(0,0,0),  // center of B in A coordinates
                (obsDirections.front()).normalized(), sigmaR, isValid, isParallel);

    std::cout <<"DLT with identical observations valid? parallel? "<< isValid <<" "<< isParallel<< " "<< std::endl<<
                v4Xhomog2.transpose() <<" comaprable with "<< obsDirections.front().normalized().transpose()<<std::endl<<std::endl;
    ...
    for(size_t jack = 0; jack< sizeof(distances)/sizeof(distances[0]); ++jack){
        ...
        Eigen::Matrix3d Rj = Eigen::AngleAxisd(30*M_PI/180, Eigen::Vector3d::UnitY()).toRotationMatrix(); // j to global
        ...
        std::cout <<"DLT with observations of point at "<<ptini.transpose() << " valid? parallel? "<< isValid <<" "<<
                    isParallel<< std::endl << v4Xhomog2.transpose() <<std::endl;

        std::vector<Eigen::Vector3d> allObs(2);
        allObs[0] = abrhoi;
        allObs[1] = abrhoj;
        std::vector<Sophus::SE3d> se3_CWs(2);
...

    {
        ...
        Eigen::Vector3d ptini(cos(15*M_PI/180)*cos(45*M_PI/180), -sin(15*M_PI/180),  cos(15*M_PI/180)*sin(45*M_PI/180));
        ...
        std::cout <<"DLT with observations of point at infinity valid? parallel? "<< isValid <<" "<<
                    isParallel<< std::endl << v4Xhomog2.transpose() <<std::endl;
...

    {
        ...
        std::cout <<"DLT with observations of point with pure rotation valid? parallel? "<< isValid <<" "<<
                    isParallel<< std::endl << v4Xhomog2.transpose() <<std::endl;
...

    {
        ...
        std::cout <<"DLT with observations of point with no motion rotation valid? parallel? "<< isValid <<" "<<
                    isParallel<< std::endl << v4Xhomog2.transpose() <<std::endl;
