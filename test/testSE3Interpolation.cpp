...
void testSE3Interpolation()
{
    ...
    std::vector<Sophus::SE3d > q02n;
    std::vector<double> times;
    double outputFreq = 100;
    std::vector<Eigen::Matrix<double, 4,4 > > samplePoses;
    std::vector<Eigen::Matrix<double, 10, 1> > samples;
    Eigen::Matrix<double, 3, 1> gw= Eigen::Matrix<double, 3, 1>::Zero();
    for(double time= tStart; time<=tEnd; time+=interval){
        times.push_back(time);
        q02n.push_back(Sophus::SE3d(Eigen::Quaterniond(1,0,0,0), Eigen::Vector3d(0.5*aing*time*time, 0,0)));
    }
    InterpolateIMUData(q02n, times, outputFreq,
                       samplePoses, samples,
                       gw);
    ...
    for(size_t jack= removeEnds; jack<samples.size() - removeEnds;++jack){
        ...
        assert((samples[jack].segment<3>(4) - Eigen::Vector3d(0,0, wing)).lpNorm<Eigen::Infinity>() <1e-8);
