...
// It is observed that RungeKutta may perform worse than Euler's implementation for forward + backward propagation
TEST(IMUOdometry, BackwardIntegration)
{
    ...
    Eigen::Matrix<double, 27, 1> vTgTsTa = 1e-2*(Eigen::Matrix<double, 27, 1>::Random());
    for( int jack =0; jack<3; ++jack)
    {
        vTgTsTa[jack*4] +=1;
        vTgTsTa[jack*4 + 18] += 1;
    }
    Eigen::Matrix<double, okvis::ceres::ode::OdoErrorStateDim, okvis::ceres::ode::OdoErrorStateDim>* P_ptr = 0;
    Eigen::Matrix<double, okvis::ceres::ode::OdoErrorStateDim, okvis::ceres::ode::OdoErrorStateDim>* F_tot_ptr = 0;
    ...
    for(int jack =0; jack<1000; ++jack){
        ...
        imuMeasurements.push_back(ImuMeasurement(okvis::Time(jack*dt), ImuSensorReadings(gyr, acc)));
    }
    ...
    IMUOdometry::propagation_RungeKutta(imuMeasurements,
                                        imuParams,
                                        T_WB, sb, vTgTsTa, imuMeasurements.begin()->timeStamp,
                                        imuMeasurements.rbegin()->timeStamp);

    IMUOdometry::propagationBackward_RungeKutta(imuMeasurements,
                                                imuParams,
                                                T_WB, sb, vTgTsTa, imuMeasurements.rbegin()->timeStamp,
    ...
    for(auto iter= imuMeasurements.begin(); iter!= imuMeasurements.end(); ++iter){
        ...
        okvis::ceres::ode::integrateOneStep_RungeKutta(
                    ...
                    dt, p_WS_W, q_WS, sb, vTgTsTa);
        iterLast = iter;
    }

    //    std::cout <<"states after forward integration: q_WS, p_WS_W, speed and bias"<<std::endl;
    //    std::cout<< q_WS.w()<<" "<<q_WS.x()<<" "<<q_WS.y()<<" "<<q_WS.z()<<std::endl;
    //    std::cout<< p_WS_W.transpose() << std::endl;
    //    std::cout<< sb.transpose()<< std::endl;


    //backward
    auto iterRLast = imuMeasurements.rbegin();
    for(auto iterR= imuMeasurements.rbegin(); iterR!= imuMeasurements.rend(); ++iterR){
        if(iterR == imuMeasurements.rbegin())
            continue;
        okvis::ceres::ode::integrateOneStepBackward_RungeKutta(
                    iterR->measurement.gyroscopes, iterR->measurement.accelerometers,
                    iterRLast->measurement.gyroscopes, iterRLast->measurement.accelerometers, g,
                    sigma_g_c, sigma_a_c, sigma_gw_c, sigma_aw_c,
                    dt, p_WS_W, q_WS, sb, vTgTsTa);
        iterRLast = iterR;
    }
    ...
    IMUOdometry::propagation(imuMeasurements,
                             imuParams,
                             T_WB, tempV_WS, iem, imuMeasurements.begin()->timeStamp,
                             imuMeasurements.rbegin()->timeStamp);

    IMUOdometry::propagationBackward(imuMeasurements,
                                     imuParams,
                                     T_WB, tempV_WS, iem, imuMeasurements.rbegin()->timeStamp,
                                     imuMeasurements.begin()->timeStamp);
    p_WS_W = T_WB.r();
    q_WS = T_WB.q();
    sb.head<3>() = tempV_WS;
    ...
    ASSERT_TRUE(fabs(q_WS2.w() - q_WS1.w())<1e-8 && fabs(q_WS2.x() - q_WS1.x())<1e-8 &&
    ...
    ASSERT_TRUE(fabs(q_WS0.w() - q_WS1.w())<1e-8 && fabs(q_WS0.x() - q_WS1.x())<1e-8 &&
    ...
    ASSERT_TRUE(fabs(q_WS0.w() - q_WS3.w())<1e-6 && fabs(q_WS0.x() - q_WS3.x())<1e-6 &&
...
TEST(Eigen, SuperDiagonal)
{
    ...
    ASSERT_LT( (superdiagonal(M) - subdiagonal(M.transpose())).norm(), 1e-9);
...
/// test and compare the propagation for both states and covariance by both the classic RK4 and okvis's state transition method
TEST(IMUOdometry, IMUCovariancePropagation)
{
    using namespace Eigen;
    using namespace okvis;
    bool bVarianceForShapeMatrices = false; //use positive variance for elements in Tg Ts Ta?
    bool bVerbose = true; //print the covariance and jacobian results
    bool bNominalTgTsTa = true; // use nominal values for Tg Ts Ta, i.e., identity, zero, identity?
    ...
    double sigma_aw_c = 2e-3;
    const double dt = 0.1;
    Eigen::Vector3d p_WS_W0(vio::gauss_rand(0, 1),vio::gauss_rand(0, 1),vio::gauss_rand(0, 1));
    ...
    Eigen::Matrix<double, okvis::ceres::ode::OdoErrorStateDim, okvis::ceres::ode::OdoErrorStateDim> P1;
    P1.setIdentity();
    if(!bVarianceForShapeMatrices)
        P1.bottomRightCorner<27, 27>().setZero();
    Eigen::Matrix<double, okvis::ceres::ode::OdoErrorStateDim, okvis::ceres::ode::OdoErrorStateDim> F_tot;
    F_tot.setIdentity();
    ...
    for(auto iter= imuMeasurements.begin(); iter!= imuMeasurements.end(); ++iter){
        ...
        okvis::ceres::ode::integrateOneStep_RungeKutta(
                    ...
                    iter->measurement.gyroscopes, iter->measurement.accelerometers, g,
                    sigma_g_c, sigma_a_c, sigma_gw_c, sigma_aw_c,
                    dt, p_WS_W, q_WS, sb, vTgTsTa, &P1, &F_tot);
        iterLast = iter;
    }
    ...
    std::cout <<"time used by RK4 forward propagtion with covariance "<<timeElapsed<< std::endl;

    if(bVerbose){
        std::cout<< "q_WS " << q_WS1.w()<<" "<<q_WS1.x()<<" "<<q_WS1.y()<<" "<<q_WS1.z()<<std::endl;
        std::cout<< "p_WS_W " << p_WS_W1.transpose()<<std::endl;
        std::cout <<"speed and bias "<< sb1.transpose()<<std::endl;

        std::cout<< "cov(r_s^w, \\phi^w, v_s^w, b_g, b_a, T_g, T_s, T_a), its diagonal sqrt "<<std::endl;
        std::cout<< sqrtDiagCov1.transpose()<<std::endl;
        std::cout<<"Jacobian superdiagonal "<<std::endl;
        std::cout<< superdiagonal(F_tot).transpose()<<std::endl;
    ...
    P2.setIdentity();
    if(!bVarianceForShapeMatrices)
        P2.bottomRightCorner<27, 27>().setZero();
    F_tot.setIdentity();
    ...
    int numUsedImuMeasurements = IMUOdometry::propagation(imuMeasurements, imuParams,T_WS, tempV_WS,
                                                          tempIEM, imuMeasurements.begin()->timeStamp,
                                                          imuMeasurements.rbegin()->timeStamp, &P2, &F_tot);
    sb.head<3>()= tempV_WS;
    ...
    std::cout <<"time used by huai trapezoid rule forward propagtion with covariance "<<timeElapsed<< std::endl;
    if(bVerbose){
        std::cout <<"numUsedMeas " <<numUsedImuMeasurements<<" totalMeas "<< (int)imuMeasurements.size()<<std::endl;
        std::cout<< "q_WS " << q_WS2.w()<<" "<<q_WS2.x()<<" "<<q_WS2.y()<<" "<<q_WS2.z()<<std::endl;
    ...
    numUsedImuMeasurements = okvis::ceres::ImuError::propagation(imuMeasurements, imuParams,T_WS, sb,
                                                          imuMeasurements.begin()->timeStamp,
    ...
    std::cout <<"time used by leutenegger forward propagtion with covariance "<<timeElapsed<< std::endl;
    if(bVerbose){
        std::cout <<"numUsedMeas " <<numUsedImuMeasurements<<" totalMeas "<< (int)imuMeasurements.size()<<std::endl;
        std::cout<< "q_WS " << leuten_q_WS.w()<<" "<<leuten_q_WS.x()<<" "<<leuten_q_WS.y()<<" "<<leuten_q_WS.z()<<std::endl;
        ...
        std::cout<<"Jacobian diagonal "<<std::endl;
        std::cout<< superdiagonal(leutenF).transpose()<<std::endl;
        std::cout<<"Jacobian first row "<<std::endl;
        std::cout<< leuten_firstRow.transpose() <<std::endl;
    }

    /// method 4 : okvis propagation leutenegger's implementation corrected by huai
    T_WS= okvis::kinematics::Transformation(p_WS_W0,q_WS0);
    sb =sb0;
    ...
    numUsedImuMeasurements = okvis::IMUOdometry::propagation_leutenegger_corrected(imuMeasurements, imuParams,T_WS, sb,
                                                          imuMeasurements.begin()->timeStamp,
    ...
    std::cout <<"time used by leutenegger corrected forward propagtion with covariance "<<timeElapsed<< std::endl;
    if(bVerbose){
        std::cout <<"numUsedMeas " <<numUsedImuMeasurements<<" totalMeas "<< (int)imuMeasurements.size()<<std::endl;
        std::cout<< "q_WS " << leuten_q_WS2.w()<<" "<<leuten_q_WS2.x()<<" "<<leuten_q_WS2.y()<<" "<<leuten_q_WS2.z()<<std::endl;
        ...
        std::cout<<"Jacobian diagonal "<<std::endl;
        std::cout<< superdiagonal(leutenF2).transpose()<<std::endl;
        std::cout<<"Jacobian first row "<<std::endl;
    ...
    sb =sb0;
    double time_pair[2] = {imuMeasurements.begin()->timeStamp.toSec(), imuMeasurements.rbegin()->timeStamp.toSec()};
    std::vector<Eigen::Matrix<double, 7,1> > measurements;
    Eigen::Matrix<double, 7, 1> meas;
    for(auto iter= imuMeasurements.begin(); iter!= imuMeasurements.end(); ++iter){
        meas<<iter->timeStamp.toSec(), iter->measurement.gyroscopes, iter->measurement.accelerometers;
        measurements.push_back(meas);
    }

    Eigen::Matrix<double, 6,1> gwomegaw;
    gwomegaw.setZero();
    Eigen::Vector3d g_W = - imuParams.g * Eigen::Vector3d(0, 0, 6371009).normalized();
    gwomegaw.head<3>() = g_W;

    Eigen::Matrix<double, 12, 1> q_n_aw_babw;
    q_n_aw_babw<<  pow(imuParams.sigma_a_c, 2),  pow(imuParams.sigma_a_c, 2), pow(imuParams.sigma_a_c, 2),
            pow(imuParams.sigma_g_c, 2), pow(imuParams.sigma_g_c, 2), pow(imuParams.sigma_g_c, 2),
            pow(imuParams.sigma_aw_c, 2),  pow(imuParams.sigma_aw_c, 2), pow(imuParams.sigma_aw_c, 2),
            pow(imuParams.sigma_gw_c, 2), pow(imuParams.sigma_gw_c, 2), pow(imuParams.sigma_gw_c, 2);

    Sophus::SE3d pred_T_s2_to_w;
    Eigen::Matrix<double, 3,1> pred_speed_2;
    timing::Timer simpleTimer("simple",false);
    okvis::ceres::ode::predictStates(T_s1_to_w, sb, time_pair,
                              measurements, gwomegaw, q_n_aw_babw,
                              &pred_T_s2_to_w, &pred_speed_2, &P3, vTgTsTa);
    timeElapsed=simpleTimer.stop();

    std::cout <<"time used by 1st order propagtion with covariance "<<timeElapsed<< std::endl;
    ...
    if(bVerbose){
        std::cout<< "q_WS " << q_WS3.w()<<" "<<q_WS3.x()<<" "<<q_WS3.y()<<" "<<q_WS3.z()<<std::endl;
    ...
    // 1,2 states
    EXPECT_TRUE(fabs(q_WS2.w() - q_WS1.w())<1e-5 && fabs(q_WS2.x() - q_WS1.x())<1e-5 &&
    ...
    EXPECT_TRUE(fabs(q_WS3.w() - q_WS1.w())<5e-2 && fabs(q_WS3.x() - q_WS1.x())<5e-2 &&
    ...
    EXPECT_LT((sqrtDiagCov2.segment<3>(3) - sqrtDiagCov1.segment<3>(3)).norm(), 2e-2);
    EXPECT_LT((sqrtDiagCov2.segment<3>(6) - sqrtDiagCov1.segment<3>(6)).norm(), 10);

    EXPECT_LT((sqrtDiagCov2.segment<3>(9) - sqrtDiagCov1.segment<3>(9)).norm(), 1e-6);
    EXPECT_LT((sqrtDiagCov2.segment<3>(12) - sqrtDiagCov1.segment<3>(12)).norm(), 1e-6);

    EXPECT_LT((sqrtDiagCov2.segment<9>(15) - sqrtDiagCov1.segment<9>(15)).norm(), 1e-6);
    EXPECT_LT((sqrtDiagCov2.segment<9>(24) - sqrtDiagCov1.segment<9>(24)).norm(), 1e-6);
    EXPECT_LT((sqrtDiagCov2.segment<9>(33) - sqrtDiagCov1.segment<9>(33)).norm(), 1e-6);

    //1,2 jacobian
    EXPECT_LT((firstRow1.head<3>() - firstRow2.head<3>()).norm()/firstRow1.head<3>().norm(), 1e-3);
    EXPECT_LT((firstRow1.segment<6>(3) - firstRow2.segment<6>(3)).norm()/firstRow1.segment<6>(3).norm(), 1e-3);
    EXPECT_LT((firstRow1.segment<6>(9) - firstRow2.segment<6>(9)).norm()/firstRow1.segment<6>(9).norm(), 1e-3);
    EXPECT_LT((firstRow1.segment<9>(15) - firstRow2.segment<9>(15)).norm()/firstRow1.segment<9>(15).norm(), 5e-3);
    EXPECT_LT((firstRow1.segment<9>(24) - firstRow2.segment<9>(24)).norm()/firstRow1.segment<9>(24).norm(), 1e-3);
    EXPECT_LT((firstRow1.segment<9>(33) - firstRow2.segment<9>(33)).norm()/firstRow1.segment<9>(33).norm(), 1e-3);

    //1,3 covariance
    EXPECT_LT((sqrtDiagCov3.head<3>() - sqrtDiagCov1.head<3>()).norm()/sqrtDiagCov3.head<3>().norm(), 5e-2);
    EXPECT_LT((sqrtDiagCov3.segment<3>(3) - sqrtDiagCov1.segment<3>(3)).norm()/sqrtDiagCov3.segment<3>(3).norm(), 5e-2);
    EXPECT_LT((sqrtDiagCov3.segment<3>(6) - sqrtDiagCov1.segment<3>(6)).norm()/sqrtDiagCov3.segment<3>(6).norm(), 5e-2);

    EXPECT_LT((sqrtDiagCov3.segment<3>(9) - sqrtDiagCov1.segment<3>(9)).norm(), 1e-6);
    EXPECT_LT((sqrtDiagCov3.segment<3>(12) - sqrtDiagCov1.segment<3>(12)).norm(), 1e-6);
}
...
TEST(IMUOdometry, InitPoseFromImu)
{
    ...
    // align with ez_W:
    Eigen::Vector3d ez_W(0.0, 0.0, 1.0); // Negative gravity direction, i.e., Z direction, in the local frame
