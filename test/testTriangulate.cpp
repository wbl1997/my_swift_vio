...
// test AutoDiffCostFunction and AddResidualBlock dimensions with Powell's minimization problem
// compare with examples/powell.cc in ceres
...
TEST(Triangulate, Powell) {
  ...
  problem.AddResidualBlock(new AutoDiffCostFunction<Fall, 4, 4>(new Fall),
                           NULL,
                           x);
...
TEST(Triangulate, AllMethods)
{
    ...
    for(int i=0; i<trials; ++i){
#if 1
        int N=100;
        Eigen::Vector3d point(1.5*(rand()%N)/N,3*(rand()%N)/N,25*(rand()%N)/N);
        ...
        so3N.normalize();
        vse3CFromW[1]= Sophus::SE3d(so3N, Eigen::Vector3d(1.0, -0.3*(rand()%N)/N, 3.0));
        Eigen::Quaterniond so3P(4.5, 0.8,1.0,1.4*(rand()%N)/N);
        so3P.normalize();
        vse3CFromW[2]=Sophus::SE3d(so3P,Eigen::Vector3d(1.0, -0.2*(rand()%N)/N, -5.0));
        for (int i=0; i<3; ++i){
            Eigen::Vector3d v3Cam = vse3CFromW[i]* point;
            if(v3Cam[2] < 0){
//                std::cerr<<v3Cam[2]<<std::endl;
//                std::cerr<<"Point "<<i<<" is behind the image"<<std::endl;
                continue;
            }
            vV2ImPlane[i]=v3Cam.head<2>()/v3Cam[2]+ Vector2d(1,1)*(rand()%10)/1000;
        }
#else
        ...
        for(int zinc=3; zinc< num_methods; ++zinc)
            res[zinc]= res[1];
...
#endif
        // cout<<"estimated point by 3 methods"<<endl<< res<<endl<<res2<<endl<<res3<<endl;
        for(int zinc=0; zinc<num_methods;++zinc){
            deviation[zinc][i]= (res[zinc]-point).transpose()*(res[zinc]-point);
//            cout<<deviation[zinc][i]<<" ";
        }
//        cout<<std::endl;
    }
    std::cout<<"SquaredResidual of DLT, 2 view MP, multiview MP, GN refined 2view MP, LM AD, Dogleg AD, LM anal, Dogleg anal"<<endl;
