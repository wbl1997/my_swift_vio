...
void ImuOdometryGtsam(const Eigen::Vector3d& p_WS_W0,
  ...
  *p_WS_W1 = p_WS_W0 + sb0.head<3>() * combinedPim->deltaTij() +
  ...
  // jacobian of the OKVIS error \xi relative to the reordered gtsam error state at
  // t_j
  Eigen::Matrix<double, 15, 15> dxi_deta_j =
      Eigen::Matrix<double, 15, 15>::Identity();
  dxi_deta_j.block<3, 3>(0, 0) = q_WS1->toRotationMatrix();
  dxi_deta_j.block<3, 3>(3, 3) = q_WS1->toRotationMatrix();


  // reorder gtsam error state from [ori, pos, vel, ba, bg] to [pos, ori, vel, bg, ba].
  Eigen::Matrix<double, 15, 15> permuteToOkvis =
      Eigen::Matrix<double, 15, 15>::Zero();
  permuteToOkvis.block<3, 3>(0, 3) = Eigen::Matrix3d::Identity();
  permuteToOkvis.block<3, 3>(3, 0) = Eigen::Matrix3d::Identity();
  permuteToOkvis.block<3, 3>(6, 6) = Eigen::Matrix3d::Identity();
  permuteToOkvis.block<3, 3>(9, 12) = Eigen::Matrix3d::Identity();
  permuteToOkvis.block<3, 3>(12, 9) = Eigen::Matrix3d::Identity();
  ...
  Eigen::Matrix<double, 15, 15> dr_deta_iOrdered =
      permuteToOkvis * dr_deta_i * permuteToOkvis.transpose();
  Eigen::Matrix<double, 15, 15> dr_deta_jOrdered =
      permuteToOkvis * dr_deta_j * permuteToOkvis.transpose();

  Eigen::Matrix<double, 15, 15> dxi_deta_i =
      Eigen::Matrix<double, 15, 15>::Identity();
  dxi_deta_i.block<3, 3>(0, 0) = q_WS0.toRotationMatrix();
  dxi_deta_i.block<3, 3>(3, 3) = q_WS0.toRotationMatrix();
  ...
  *jacobian = dxi_deta_j * (-dr_deta_jOrdered.transpose()) * dr_deta_iOrdered * dxi_deta_i.transpose();

  Eigen::Matrix<double, 15, 15> dr_deta_j_inv = dr_deta_j.transpose();
  *covariance = dxi_deta_j * permuteToOkvis * dr_deta_j_inv * cov_r *
                dr_deta_j_inv.transpose() * permuteToOkvis.transpose() * dxi_deta_j.transpose();
...
//    std::cout << "Jacobian\n" << jacobianOkvis.format(commaInitFmt) << std::endl;
  ...
  std::cout << "Check topLeft covariance of okvis and gtsam\n";
  checkSelectiveRatio(covOkvis.topLeftCorner<15, 9>(),
                      covGtsam.topLeftCorner<15, 9>(), 0.35, 2, 1);
  std::cout << "Check topRight covariance of okvis and gtsam\n";
